...
const AttachmentList = ({ attachments, disabled, addAttachments, deleteAttachment, large = false, detailed = false }: AttachmentListProps): JSX.Element => {
  const iconSize = large ? 24 : 16;
  const iconColumnSize = detailed ? 24 : 8;

  const getAttachmentDownloadLink = (attachment: Attachment): string | undefined => {
  ...
  const getAttachmentIcon = (row: TableOptions<Attachment>): JSX.Element => {
    ...
    return <EdsIcon name={iconName} size={iconSize} color={tokens.colors.text.static_icons__default} />;
  };
  ...
  const getUploadedAt = (row: TableOptions<Attachment>): JSX.Element => {
    return row.value.uploadedAt ? <div>{getFormattedDateAndTime(row.value.uploadedAt)}</div> : <></>;
  };
  ...
  const determineDragAndDropRender = (): JSX.Element => {
    if (addAttachments != undefined) {
      return (
        <div>
          <DragAndDropTitle>Drag and drop to add files, or click on the button {large ? 'above' : 'below'}</DragAndDropTitle>
          <DragAndDropContainer onDrop={(event: React.DragEvent<HTMLDivElement>): void => handleDrop(event)} onDragOver={(event: React.DragEvent<HTMLDivElement>): void => handleDragOver(event)} data-testid="DnDField">
  ...
  const determineToolbarButtonRender = (): JSX.Element => {
    if (addAttachments != undefined && large === false) {
      return (
        <AddFile>
          <form>
            <StyledButton variant="ghost" disabled={disabled} data-testid={'addFiles'} onClick={handleAddFile}>
              {addIcon} Add files
            </StyledButton>
