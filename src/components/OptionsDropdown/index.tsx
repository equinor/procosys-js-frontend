...
const OptionsDropdown: React.FC<DropdownProps> = ({ disabled = false, closeOnClick = false, text = '', icon, children, variant, iconSize = 24 }: DropdownProps): JSX.Element => {
  ...
  return (
    <Container ref={containerRef}>
      <DropdownButton onClick={toggleDropdown} disabled={disabled} role="listbox" isOpen={isOpen} aria-expanded={isOpen} aria-haspopup={true} variant={variant}>
        {icon && (
          <IconContainer size={iconSize}>
            <EdsIcon name={icon} size={iconSize} color={disabled ? tokens.colors.interactive.disabled__border.rgba : tokens.colors.interactive.primary__resting.rgba} />
          </IconContainer>
        )}
        {text}
      </DropdownButton>
