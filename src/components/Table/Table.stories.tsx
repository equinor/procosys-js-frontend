...
const columns = [
  ...
  },
  {
    ...
    Cell: (rowData: TableOptions<PreservedTag>): JSX.Element => {
      return rowData.row.values.Voided === 'Voided' ? <CheckBox color="disabled" /> : <></>;
    },
  },
];
...
export const Default: Story<TableProperties<LocalTableType>> = (args: JSX.IntrinsicAttributes & TableProperties<LocalTableType>) => {
  ...
  return (
    <Wrapper>
      <Table setPageSize={setPageSize} key={id} {...args} pageSize={pageSize} columns={columns} maxRowCount={50} data={getData().slice(0, 50) || []} />
    </Wrapper>
  );
};
...
export const Filtering: Story<TableProperties<LocalTableType>> = (args: JSX.IntrinsicAttributes & TableProperties<LocalTableType>) => {
  return (
    <Wrapper>
      <Table {...args} pageSize={10} columns={filteredColumns} maxRowCount={50} data={getData().slice(0, 50) || []} />
    </Wrapper>
  );
};
