import { Cell, CellProps, Column, ColumnInstance, HeaderProps, Hooks, TableInstance, TableOptions, useFilters, useFlexLayout, usePagination, useResizeColumns, useRowSelect, useSortBy, useTable } from 'react-table';
  ...
  hooks.allColumns.push((columns) => [
    {
      ...
      width: 50,
      maxWidth: 50,
      Header: ({ getToggleAllRowsSelectedProps, disableSelectAll, rows, toggleRowSelected }: HeaderProps<Record<string, unknown>>): JSX.Element => {
        //Remove on change form selectAllprops
        const { onChange, ...propsWithoutOnChange } = getToggleAllRowsSelectedProps();
        //Function to select all rows where checkbox isn't disabled
        const overridenOnChange = (): void => {
          rows.forEach((row) => {
            if (!row.original.disableCheckbox) {
              let toggleTo = true;
              if (propsWithoutOnChange.checked || propsWithoutOnChange.indeterminate) toggleTo = false;
        ...
        return <HeaderCheckbox {...newProps} disabled={disableSelectAll} />;
      },
...
const ProcosysTable = forwardRef((props: PropsWithChildren<TableProperties<Record<string, unknown>>>, ref: any) => {
  ...
  const hooks = [];
  hooks.push(useFlexLayout, useFilters, useSortBy, usePagination, useResizeColumns, useRowSelect);
  ...
  const tableInstance = useTable<Record<string, unknown>>(
    {
      ...
      initialState: {
        ...
        pageSize: props.disablePagination ? Math.max(props.data.length, 1) : props.pageSize,
        selectedRowIds: props.selectedRows || {},
        disableSelectAll: props.disableSelectAll,
        sortBy: props.orderBy ? [props.orderBy] : [{ id: 'createdAt', desc: true }],
      },
    },
    ...hooks
  );
    ...
    useEffect(() => {
      if (rowRef.current && (rowHeights.current[index] == 40 || rowHeights.current[index] === undefined)) {
        ...
        for (let i = 0; i < children.length; i++) {
          ...
          if (child.clientHeight > maxValue) maxValue = child.clientHeight;

          const grandChildren = child.children;
          for (let ci = 0; ci < grandChildren.length; ci++) {
            const grandChild = grandChildren[ci];
            if (grandChild.clientHeight > maxValue) maxValue = grandChild.clientHeight;
          }
        }
    ...
    if (props.disablePagination) {
      ...
      if (props.clientPagination) {
        if (tableInstance.state.pageIndex > 0) {
          ...
          if (tableInstance.filteredRows.length > itemsEndIndex) itemCount = tableInstance.state.pageSize;
          else {
      ...
      {({ height, width }: AutoSizerProps): JSX.Element => (
        <div>
          ...
          <Table key={counter} {...getTableProps()}>
              ...
              headerGroups.map((headerGroup, i) => (
                ...
                >
                  {headerGroup.headers.filter((c) => c.filter).length > 0 &&
                    headerGroup.headers.map((column) => {
                      return (
                        <TableHeadFilterCell {...column.getHeaderProps()} key={column.getHeaderProps().key}>
            ...
            {/* render table */}
            <div {...getTableBodyProps()} style={{ minHeight: 120 }}>
              {props.data.length === 0 && <div>No records to display</div>}
              <List height={height - 60} itemCount={getItemCount(tableInstance)} itemSize={getRowHeight} width={width} ref={listRef} onItemsRendered={addScrollEventHandler}>
                {RenderRow}
              </List>
