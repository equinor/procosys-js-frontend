...
const TreeView = ({ rootNodes, dirtyNodeId, resetDirtyNode, selectedLibraryItem }: TreeViewProps): JSX.Element => {
  ...
  const [navigateNewJourney, setnavigateNewJourney] = useState<string | null>(null);
  ...
  const isDataReady = !!(childOldNode && parentOldNode && childNewNode && parentNewNode);
  ...
  const getNodeLink = (node: NodeData): JSX.Element => {
    ...
    const linkContent = (
      ...
      >
        {node.onClick ? (
          <NodeLink to={finalPath} isExpanded={node.isExpanded === true} isVoided={node.isVoided === true} title={node.name} onClick={(): void => selectNode(node)}>
            {node.name}
          </NodeLink>
        ) : (
          node.name
  ...
  const expandNodeRecursively = async (
    ...
    const nextNode = currentNode.children?.find((child) => decodeURIComponent(child.name) === nextNodeName);
    if (!nextNode && !currentNode.isSelected) {
      selectNode(currentNode);
    }
    if (nextNode) {
      if (nextNode && checkMounted()) {
        await expandNodeRecursively(nextNode, remainingPath.slice(1), expandNode, selectNode, setIsNodeExpanded, checkMounted);
      }
    }
  };
  ...
  const expandNodePath = async (
    ...
    if (nodeNames.length && treeData.length && checkMounted()) {
      ...
      const rootNode = treeData.find((node) => decodeURIComponent(node.name) === rootNodeName);
      if (rootNode) {
        await expandNodeRecursively(rootNode, nodeNames.slice(1), expandNode, selectNode, setIsNodeExpanded, checkMounted);
      }
    }
  };

  const storeInfoMoveNewJourney = async (newJourney: Journey): Promise<void> => {
    ...
    const childOldNode = treeData.find((node) => node.id === childOldJourneyId && node.name == childOldTitle);

    const parentOldNode = treeData.find((node) => node.id === childOldNode?.parentId);

    const parentNewNode = treeData.find((node) => node.name === parentNewName && node.id === parentNewProjectId);
    ...
    if (parentNewNode) {
      if (typeof parentNewNode.getChildren === 'function') {
        try {
          ...
          const childNewNode = children.find((child) => child.id === childOldJourneyId);
          ...
          console.error(`Error fetching children for '${parentNewNode}':`, error);
        }
      }
    }
  };
  ...
  const moveNewJourney = async (): Promise<void> => {
    if (childOldNode && parentOldNode && childNewNode && parentNewNode) {
      ...
      const fullPath = encodeURI(`${baseLibraryPath}/${parentPath}/${childPath}`);
  ...
  const resetNewJourney = async (newJourney: Journey): Promise<void> => {
    ...
    const childOldNode = treeData.find((node) => node.id === childOldJourneyId);
    ...
    if (parentOldNode) {
      ...
      const fullPath = encodeURI(`${baseLibraryPath}/${parentPath}/${journeyPath}/${childPath}`);
      ...
      try {
        if (pathToExpandTree) {
          for (const nodeId of pathToExpandTree) {
            ...
            if (node && !node.isExpanded && node.isExpanded !== false) {
  ...
  useEffect(() => {
    ...
    if (executionCount < nodeNames.length) {
      expandNodePath(nodeNames, treeData, expandNode, selectNode, setIsNodeExpanded, checkMounted).catch(console.error);
  ...
  return <TreeContainer>{treeData.map((node) => getNode(node))}</TreeContainer>;
};
