...
export const PlantContextProvider: React.FC = ({ children }): JSX.Element => {
  ...
  if (!user || !user.plants || user.plants.length === 0) {
    console.error('User has no plants assigned or user.plants is undefined');
  ...
  if (!plantInPath || plantInPath === '' || typeof plantInPath !== 'string') {
    ...
    return <ErrorComponent title={`Invalid or missing plant ID in path: ${plantInPath || 'undefined'}`} />;
  }
  ...
  const [currentPlant, setCurrentPlantInContext] = useState<PlantContextDetails>(() => {
    ...
    if (!user || !Array.isArray(user.plants)) {
      const plantsValue = user?.plants ? `Actual value of 'user.plants': ${JSON.stringify(user.plants, null, 2)}` : "'user.plants' is undefined or null.";

      throw new Error(`Invalid user object: 'user.plants' is not defined or not an array. ${plantsValue}`);
    }
    ...
    if (!plant) {
      //TODO:  to remove log in the future
      console.warn(`No plant found for path ID: ${plantInPath}. Using default fallback.`);
  ...
  const setCurrentPlant = (plantId: string): void => {
    ...
    if (plantsFiltered.length <= 0) {
      ...
      throw new InvalidParameterException(`Available plants: ${user.plants}, PlantID: ${plantId} does not exist. , plantInPath: ${plantInPath}`);
    }
  ...
  useEffect(() => {
    if (!user || !user.plants) {
      console.error('User data is not available. Cannot set current plant.');
      return;
    }
