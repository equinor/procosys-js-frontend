...
export class ProCoSysApiError extends Error {
  ...
  constructor(error: AxiosError) {
    ...
    if (Axios.isCancel(error)) {
      ...
      super('Data has been updated by another user. Please reload and start over!');
      ...
      if (isOfType<PreservationErrorResponse>(_error.response.data, 'errors')) {
        super(_error.response.data ? `${_error.response.data.errors[''][0]}` : undefined);
      } else if (isOfType<dataError>(_error.response.data, 'errors')) {
        super(_error.response.data ? `${_error.response.data.errors.IpoException}` : undefined);
      } else {
        super(_error.response.data ? `${_error.response.data}` : undefined);
      }
    } else {
