...
export const UserContextProvider: React.FC = (props): JSX.Element => {
  ...
  async function fetchPlants(): Promise<void> {
    try {
      const plantResponse = await procosysApiClient.getAllPlantsForUserAsync((c) => {
  ...
  if (errorEncountered) {
    return (
      <Container>
        <Error large title="We encountered an unexpected error, try again or contact support for further assistance" />
      </Container>
    );
  }
  ...
  if (name === '' || !plants.length) {
    return (
      <Container>
        <Error large title="You dont seem to have an account in ProCoSys - use AccessIT to request access" />
      </Container>
    );
  }
