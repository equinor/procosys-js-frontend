 ...
 */
const useClickOutsideNotifier = (cb: (originalEvent: Event) => void, targetRef: MutableRefObject<HTMLElement | null>): void => {
  const handleClickOutsideTarget = (event: any): void => {
    ...
    if (targetRef.current && !targetRef.current.contains(event.target as Node) && (!event.offsetX || event.offsetX <= event.target.clientWidth)) {
