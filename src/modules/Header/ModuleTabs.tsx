  ...
  return (
    <SubNav>
      ...
      <span onClick={props.onClick}>
        <NavLink className={({ isActive }: { isActive: boolean }) => (isActive ? 'active' : '')} to="Preservation">
          Preservation
        </NavLink>
      </span>
      ...
      {(import.meta as ImportMeta & { env: { MODE: 'development' } }).env.MODE === 'development' ? (
        <span onClick={props.onClick}>
          <NavLink className={({ isActive }: { isActive: boolean }) => (isActive ? 'active' : '')} to="libraryv2">
            Library
          </NavLink>
        </span>
      ) : (
