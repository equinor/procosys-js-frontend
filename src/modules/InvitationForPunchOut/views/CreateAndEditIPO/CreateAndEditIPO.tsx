...
const validateGeneralInfo = (info: GeneralInfoDetails, confirmationChecked?: boolean): Record<string, string> | null => {
  let errors = {};
  const { title, description, location, poType, projectName, date, startTime, endTime } = info;
  ...
  if (typeof confirmationChecked === 'boolean') {
    (!projectName || projectName === 'All projects') && (errors = { ...errors, projectName: 'Required field.' });
  ...
  if (description) {
    description.length > 4096 &&
      (errors = {
        ...errors,
        description: 'Description is too long. Maximum 4096 characters.',
      });
  }
  if (location) {
    location.length > 250 &&
...
const CreateAndEditIPO = ({
  ...
  useEffect(() => {
    setInvitationStarted(status ? !(status == IpoStatusEnum.PLANNED) : false);
  }, [status]);
  ...
  return (
    <Container>
      ...
      {currentStep == StepsEnum.Participants && <Participants participants={participants} setParticipants={setParticipants} availableRoles={availableRoles ? availableRoles : []} invitationStarted={invitationStarted} />}
      {currentStep == StepsEnum.UploadAttachments && <Attachments attachments={attachments} setAttachments={setAttachments} />}
