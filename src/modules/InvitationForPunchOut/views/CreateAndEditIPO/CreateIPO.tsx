...
const initialParticipants: Participant[] = [
  ...
  {
    organization: {
      text: OrganizationMap.get(OrganizationsEnum.ConstructionCompany) as string,
...
const CreateIPO = (): JSX.Element => {
  ...
  const [projectNameFromMain] = useState<string | null>(projectName ? decodeURIComponent(projectName) : null);
  const [commPkgNoFromMain] = useState<string | null>(commPkgNo ? decodeURIComponent(commPkgNo) : null);
  ...
  useEffect(() => {
    ...
    try {
      (async (): Promise<void> => {
        const functionalRoles = await apiClient.getFunctionalRolesAsync().then((roles) =>
  ...
  const createNewIpo = async (): Promise<void> => {
    setIsCreating(true);
    if (generalInfo.title && generalInfo.projectName && generalInfo.poType && generalInfo.date && generalInfo.startTime && generalInfo.endTime) {
      try {
