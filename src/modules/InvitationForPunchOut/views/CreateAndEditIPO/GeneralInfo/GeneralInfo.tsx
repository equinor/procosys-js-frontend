...
const GeneralInfo = ({ generalInfo, setGeneralInfo, fromMain, isEditMode, clearScope, confirmationChecked, setConfirmationChecked, errors, isDisabled }: GeneralInfoProps): JSX.Element => {
  ...
  const [date, setDate] = useState<Date | null>(generalInfo.date ? generalInfo.date : null);
  const [startTime, setStartTime] = useState<string | null>(generalInfo.startTime ? generalInfo.startTime.toString() : null);
  const [endTime, setEndTime] = useState<string | null>(generalInfo.endTime ? generalInfo.endTime.toString() : null);
    ...
    <FormContainer>
      <Column>
        <FieldContainer>
          <Dropdown
            ...
            maxHeight="300px"
            variant="form"
            text={generalInfo.projectName === 'All projects' || !generalInfo.projectName ? 'Select' : generalInfo.projectName}
          ...
          >
            {filteredProjects &&
              filteredProjects.map((projectItem, index) => {
                ...
                return (
                  <DropdownItem key={index} onClick={(event: React.MouseEvent): void => setProjectForm(event, index)}>
          ...
          {errors && errors['projectName'] && (
            <ErrorContainer>
              <EdsIcon name="error_filled" size={16} color={tokens.colors.interactive.danger__text.rgba} />
              <Typography variant="caption" color={tokens.colors.interactive.danger__text.rgba}>
                {errors['projectName']}
              </Typography>
            </ErrorContainer>
          )}
        ...
        <FieldContainer>
          <PoTypeContainer id="po-type-select">
            <SelectInput onChange={setPoTypeForm} data={poTypes} label={'Type of punch round'} disabled={isDisabled}>
      ...
      <Column>
        ...
        <DateTimeContainer>
          <div>
            <Label label={'Date'} />
            <DatePicker renderInput={(props): JSX.Element => <DateTimeField {...props} />} value={date} onChange={handleSetDate} disabled={isDisabled} />
          </div>
          <div>
            <Label label={'Start'} />
            <TimePicker renderInput={(props): JSX.Element => <DateTimeField {...props} />} value={startTime} onChange={handleSetStartTime} disabled={isDisabled} />
          </div>
          <div>
            <Label label={'End'} />
            <TimePicker renderInput={(props): JSX.Element => <DateTimeField {...props} />} value={endTime} onChange={handleSetEndTime} disabled={isDisabled} />
          </div>
        </DateTimeContainer>
        {errors && errors['time'] && (
          <ErrorContainer>
        ...
        <FieldContainer>
          <LocationContainer>
            ...
            {isEditMode ? (
              ...
              <Tooltip title="Can't be changed later" placement="bottom">
                <TeamsMeetingContainer>
                  <Checkbox
                    checked={isOnline}
                    onChange={(): void => {
        ...
        <FieldContainer>
          <ConfirmationTextContainer>
            ...
            <TextContainer>
              ...
              <br />
              <Typography variant="body_short" fontWeight={400}>
                Mechanical Completion means that the installation is built in accordance with relevant drawings and specifications. All specified tests and inspections are carried out and documented in a uniform way.
              </Typography>
            </TextContainer>
          </ConfirmationTextContainer>
