...
const Organizations: SelectItem[] = [
  ...
  {
    text: `${OrganizationMap.get(OrganizationsEnum.ConstructionCompany) as string} additional`,
    value: OrganizationsEnum.ConstructionCompany,
  },
  {
    text: `${OrganizationMap.get(OrganizationsEnum.Contractor) as string} additional`,
  ...
  {
    text: OrganizationMap.get(OrganizationsEnum.TechnicalIntegrity) as string,
...
const Participants = ({ participants, setParticipants, availableRoles, invitationStarted }: ParticipantsProps): JSX.Element => {
      ...
      try {
        (async (): Promise<void> => {
          ...
          const signerPersons = await apiClient
            .getSignerPersonsAsync(input, (cancel: Canceler) => (requestCanceler = cancel))
            .then((persons) =>
      ...
      try {
        (async (): Promise<void> => {
          ...
          const persons = await apiClient
            .getPersonsAsync(input, (cancel: Canceler) => (requestCanceler = cancel))
            .then((persons) =>
              persons.map((person): SelectItem => {
                return {
                  text: nameCombiner(person.firstName, person.lastName),
  ...
  const getOrganizationText = (organization: string, sortKey: number): string | undefined => {
    ...
    const organizationIsContractorOrContructionCompany = organization === OrganizationMap.get(OrganizationsEnum.Contractor) || organization === OrganizationMap.get(OrganizationsEnum.ConstructionCompany);
      ...
      <FormContainer>
        <ParticipantRowsContainer>
          {participants.map((p, index) => {
            const disableEditing: boolean = invitationStarted ? (p.signedAt ? true : false) : false;
            return (
              <React.Fragment key={`participant_${index}`}>
                <div>
                  <SelectInput onChange={(value): void => setOrganization(value, index)} data={Organizations} label={'Organization'} disabled={index < 2 || disableEditing}>
                    {p.organization.text ? getOrganizationText(p.organization.text, index) : 'Select'}
                  </SelectInput>
                </div>
                <div>
                  <SelectInput onChange={(value): void => setType(value, index)} data={ParticipantType} label={'Type'} disabled={disableEditing || p.organization.value == OrganizationsEnum.External}>
                    {p.type}
                  </SelectInput>
                </div>
                    ...
                    >
                      ...
                      {!isLoading &&
                        filteredPersons.map((person, i) => {
                          return (
                            <DropdownItem key={i} onClick={(event: React.MouseEvent): void => setPersonOnParticipant(event, i, index)}>
                              <div>{person.text}</div>
                            </DropdownItem>
                          );
                        })}
                ...
                {p.type == ParticipantType[0].text && (
                  <div>
                    <RoleSelector selectedRole={p.role} onChange={(value): void => setRoleOnParticipant(value, index)} roles={getRolesCopy()} label={'Role'} disabled={disableEditing}>
                      {p.role ? (
                        <Tooltip title={getDisplayText(index)} arrow={true} enterDelay={200} enterNextDelay={100}>
                          <div className="overflowControl">{getDisplayText(index)}</div>
                        </Tooltip>
                      ) : (
                        'Select'
                      )}
