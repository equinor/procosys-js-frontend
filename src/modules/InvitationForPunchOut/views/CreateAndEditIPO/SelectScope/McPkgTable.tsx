...
const McPkgTable = forwardRef(({ selectedMcPkgScope, setSelectedMcPkgScope, projectName, commPkgNo }: McPkgTableProps, ref): JSX.Element => {
  ...
  useEffect(() => {
    ...
    (async (): Promise<void> => {
      try {
        const availableMcPkgs = await apiClient.getMcPkgsAsync(projectName, commPkgNo).then((mcPkgs) =>
          mcPkgs.map((mcPkg): McPkgRow => {
            return {
              ...
              operationHandoverStatus: mcPkg.operationHandoverStatus,
  ...
  const unselectMcPkg = (mcPkgNo: string): void => {
    ...
    if (selectedIndex > -1) {
      ...
      const newSelectedMcPkgScope = {
        system: newSelected.length > 0 ? selectedMcPkgScope.system : null,
    ...
    if (!selectedMcPkgScope.system) {
      setSelectedMcPkgScope((selectedScope: McScope) => {
        return {
          ...
          multipleDisciplines: multipleDisciplines([...selectedScope.selected, ...rowData]),
  ...
  const removeAllSelectedMcPkgsInScope = (): void => {
    ...
    setSelectedMcPkgScope({
      system: newSelectedMcPkgs.length > 0 ? newSelectedMcPkgs[0].system : null,
      ...
      {!isLoading && (
        <McPkgTableContainer>
          <ProcosysTable
            ...
            pageIndex={0}
            rowSelect={true}
            onSelectedChange={(rowData: McPkgRow[], ids: any): void => {
              rowSelectionChangedMc(rowData, ids);
            }}
            selectedRows={availableMcPkgs
              .filter((x: McPkgRow) => x.tableData?.isSelected)
