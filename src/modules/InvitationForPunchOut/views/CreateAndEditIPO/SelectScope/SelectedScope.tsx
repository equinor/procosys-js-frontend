...
const SelectedScope = ({ selectedCommPkgs = [], removeCommPkg, selectedMcPkgs = [], removeMcPkg, multipleDisciplines, isDisabled }: SelectedScopeProps): JSX.Element => {
    ...
    return (
      <Accordion.Item key={commPkg.commPkgNo}>
        ...
        <Accordion.Panel>
          <AccordionContent>
            <div>
              <Typography variant="caption">Comm pkg description</Typography>
              <Typography variant="body_long">{commPkg.description}</Typography>
            </div>
            <div>
              <Typography variant="caption">Comm pkg status</Typography>
              <Typography variant="body_long">{commPkg.status}</Typography>
            </div>
          </AccordionContent>
        </Accordion.Panel>
      </Accordion.Item>
    ...
    return (
      <Accordion.Item key={mcPkg.mcPkgNo}>
        ...
        <Accordion.Panel>
          <AccordionContent>
            <div>
              <Typography variant="caption">Mc pkg description</Typography>
              <Typography variant="body_long">{mcPkg.description}</Typography>
            </div>
            <div>
              <Typography variant="caption">Status</Typography>
              <Typography variant="body_long">{mcPkg.status}</Typography>
            </div>
            <div>
              <Typography variant="caption">M-01 date</Typography>
              <Typography variant="body_long">{mcPkg.m01 ? getFormattedDate(mcPkg.m01) : '-'}</Typography>
            </div>
            <div>
              <Typography variant="caption">M-02 date</Typography>
              <Typography variant="body_long">{mcPkg.m02 ? getFormattedDate(mcPkg.m02) : '-'}</Typography>
            </div>
          </AccordionContent>
        </Accordion.Panel>
      </Accordion.Item>
  ...
  return (
    <SelectedScopeContainer>
      ...
      <TextContainer>
        <Typography variant="body_long">{numberOfPackagesSelected()} package(s) selected</Typography>
        {multipleDisciplines && multipleDisciplinesWarning}
      </TextContainer>

      {selectedCommPkgs.length > 0 && <Accordion>{selectedCommPkgs.map((commPkg) => createSectionForCommPkg(commPkg))}</Accordion>}
      {selectedMcPkgs.length > 0 && <Accordion>{selectedMcPkgs.map((mcPkg) => createSectionForMcPkg(mcPkg))}</Accordion>}
    </SelectedScopeContainer>
  );
};
