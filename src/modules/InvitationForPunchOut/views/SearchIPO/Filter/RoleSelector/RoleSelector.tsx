...
const RoleSelector = ({ functionalRoleCode, onChange, roles }: RoleSelectorProps): JSX.Element => {
  ...
  return (
    <Container>
      <Dropdown
        ...
        Icon={
          selectedRole ? (
            <div onClick={(e): void => clearRole(e)}>
      ...
      >
        ...
        {roles.length > 0 &&
          roles.map((role, i) => {
            return (
              <DropdownItem key={i} onClick={(event: React.MouseEvent): void => setRole(event, i)}>
                <div>{role.text}</div>
              </DropdownItem>
            );
          })}
