...
const SavedFilters = (props: SavedFiltersProps): JSX.Element => {
  ...
  const saveFilter = async (): Promise<void> => {
    ...
    try {
      await apiClient.addSavedIPOFilter(props.project.id === -1 ? null : props.project.name, newFilterTitle, newFilterIsDefault, JSON.stringify(props.ipoFilter));
  ...
  const onSetDefaultValue = async (defaultValue: boolean, index: number): Promise<void> => {
    if (props.savedIPOFilters) {
      ...
      defaultValue ? showSnackbarNotification('Filter is set to default.', 5000) : showSnackbarNotification('Filter is no longer default.', 5000);
      props.refreshSavedIPOFilters();
    }
  };
  ...
  return (
    <Container>
      ...
      <div
        style={{
          padding: 'calc(var(--grid-unit) * 2) 0px calc(var(--grid-unit) * 2)',
        }}
      >
        <Button
          onClick={(): void => {
            ...
            return (
              <React.Fragment key={`filter._${index}`}>
                <Row isSelectedFilter={index == props.selectedFilterIndex}>
                  <Link onClick={(): void => onSelectFilter(index)}>{filter.title}</Link>
                  <div
                    style={{
                      display: 'flex',
                      flexDirection: 'row',
