...
import { Container, ContentContainer, DropdownItem, FilterContainer, Header, HeaderContainer, IconBar, LeftPartOfHeader, StyledButton, TooltipText } from './SearchIpo.style';
...
const SearchIPO = (): JSX.Element => {
  ...
  const { apiClient, project, setCurrentProject, availableProjects } = useInvitationForPunchOutContext();
  ...
  const updateSavedFilters = async (): Promise<void> => {
    ...
    try {
      const response = await apiClient.getSavedIPOFilters(project.name === 'All projects' ? null : project.name);
    ...
    if (project) {
      ...
      try {
        ...
        return await apiClient
          .getIPOs(project.name, page, pageSize, orderBy, orderDirection, filter, (c) => {
  ...
  const exportInvitationsToExcel = async (): Promise<void> => {
    if (project) {
      try {
        ...
        await apiClient.exportInvitationsToExcel(project.name, orderByField, orderDirection, filter).then((response) => {
      ...
      <ContentContainer withSidePanel={displayFilter}>
        <HeaderContainer ref={moduleHeaderContainerRef}>
          <LeftPartOfHeader>
            ...
            <IconBar>
              {
                <Dropdown maxHeight="300px" text={project ? project.name : 'Select project'} onFilter={setFilterForProjects}>
                  {isLoading && (
                    <div
                      style={{
              ...
              <Link to={project ? `CreateIPO/${project.name}` : 'CreateIPO'}>
                <Button variant="ghost">{addIcon} New IPO</Button>
              </Link>
            </IconBar>
          </LeftPartOfHeader>
          <Tooltip
