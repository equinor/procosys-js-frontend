...
const SignatureButtons = ({ participant, status, loading, setLoading, complete, accept, sign, unaccept, uncomplete, unsign, isUsingAdminRights }: SignatureButtonsProps): JSX.Element => {
  const handleButtonClick = async (buttonAction: () => Promise<any>): Promise<void> => {
    setLoading(true);
    await buttonAction();
    setLoading(false);
  };

  const handleSignButtonClick = async (buttonAction: () => Promise<any>): Promise<void> => {
    setLoading(true);
    await buttonAction();
    setLoading(false);
  };
  ...
  switch (participant.organization) {
    case OrganizationsEnum.Contractor:
      if (participant.sortKey === 0) {
        ...
        } else if ((participant.isSigner || isUsingAdminRights) && status === IpoStatusEnum.COMPLETED) {
          return (
            <SignatureButton
              name={'Uncomplete'}
              onClick={(): Promise<void> =>
        ...
        if ((participant.isSigner || isUsingAdminRights) && participant.signedBy) {
          return getUnsignButton();
        } else if (participant.isSigner && status !== IpoStatusEnum.CANCELED) {
    ...
    case OrganizationsEnum.ConstructionCompany:
      if (participant.sortKey === 1) {
        if ((participant.isSigner || isUsingAdminRights) && status === IpoStatusEnum.ACCEPTED) {
          return (
            <SignatureButton
        ...
        } else if (participant.isSigner && status === IpoStatusEnum.COMPLETED) {
          return (
            <SignatureButtonWithTooltip
