      ...
      <CustomTable>
        ...
        <Body>
          {commPkgScope.length > 0 ? (
            commPkgScope.map((commPkg: CommPkgScope, index: number) => (
              <Row key={index} as="tr">
                <Cell
                  as="td"
                  style={{
                ...
                >
                  <Typography href={getCommPkgUrl(commPkg.commPkgNo)} variant="body_short" link>
                    {commPkg.commPkgNo}
                  </Typography>
                </Cell>
                <Cell as="td" style={{ verticalAlign: 'middle' }}>
                  <Typography variant="body_short">{commPkg.description}</Typography>
                </Cell>
                <Cell as="td" style={{ verticalAlign: 'middle' }}>
                  <Typography variant="body_short">{commPkg.status}</Typography>
                </Cell>
              </Row>
            ))
          ) : (
