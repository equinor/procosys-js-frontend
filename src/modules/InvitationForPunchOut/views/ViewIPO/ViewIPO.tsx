import { CompleteAcceptIPODto, AttendedStatusDto, NotesDto, IpoApiError, SignIPODto } from '../../http/InvitationForPunchOutApiClient';
...
const ViewIPO = (): JSX.Element => {
  ...
  const [isUsingAdminRights, setIsUsingAdminRights] = useState<boolean>(false);
  ...
  const updateAttendedStatus = async (attendedStatus: AttendedStatusDto): Promise<any> => {
    try {
      await apiClient.updateAttendedStatus(ipoId, attendedStatus);
      invitation && analytics.trackUserAction(IpoCustomEvents.UPDATED_ATTENDED_STATUS, { project: invitation.projectName, type: invitation.type });
      await getInvitation();
  ...
  const updateNotes = async (attendedNotes: NotesDto): Promise<any> => {
    try {
      ...
      invitation && analytics.trackUserAction(IpoCustomEvents.UPDATED_ATTENDED_NOTES, { project: invitation.projectName, type: invitation.type });
      await getInvitation();
  ...
  const completePunchOut = async (participant: Participant): Promise<any> => {
    const signer = participant.person ? participant.person : participant.functionalRole ? participant.functionalRole : undefined;
  ...
  const uncompletePunchOut = async (participant: Participant): Promise<any> => {
    ...
    try {
      await apiClient.uncompletePunchOut(ipoId, invitation.rowVersion, participant.rowVersion);
  ...
  const unacceptPunchOut = async (participant: Participant): Promise<any> => {
    ...
    try {
      await apiClient.unacceptPunchOut(ipoId, invitation.rowVersion, participant.rowVersion);
  ...
  const unsignPunchOut = async (participant: Participant): Promise<any> => {
    ...
    try {
      await apiClient.unsignPunchOut(ipoId, participant.id, participant.rowVersion);
  ...
  const deletePunchOut = async (): Promise<any> => {
    if (invitation) {
      try {
        ...
        showSnackbarNotification('Invitation for punch-out is deleted.');
        navigate('/');
      } catch (error) {
        if (!(error instanceof IpoApiError)) return;
        console.error(error.message, error.data);
      ...
      {loading ? (
        ...
        <InvitationContainer>
          <ViewIPOHeader
            ipoId={ipoId}
            steps={steps}
            isCancelled={invitation.status === IpoStatusEnum.CANCELED}
            currentStep={currentStep}
            title={invitation.title}
            organizer={`${invitation.createdBy.firstName} ${invitation.createdBy.lastName}`}
            participants={invitation.participants}
            isEditable={invitation.status == IpoStatusEnum.PLANNED || (isAdmin && isUsingAdminRights)}
            showEditButton={invitation.canEdit || (isAdmin && isUsingAdminRights)}
            canDelete={invitation.canDelete || (invitation.status === IpoStatusEnum.CANCELED && isAdmin && isUsingAdminRights) || (invitation.status === IpoStatusEnum.SCOPEHANDEDOVER && isAdmin && isUsingAdminRights)}
          ...
          <InvitationContentContainer>
            <TabsContainer>
              ...
              <CommentsIconContainer>
                {!showComments && (
                  <Button variant="ghost_icon" onClick={(): void => setShowComments((show) => !show)}>
