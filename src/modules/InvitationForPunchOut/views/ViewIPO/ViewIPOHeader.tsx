...
const ViewIPOHeader = ({
  ...
  const confirmCancelIpo = (): void => {
    showModalDialog('Cancel IPO', <div>Are you sure you want to cancel the IPO and send a cancellation to invited participants?</div>, '18vw', 'No', null, 'Yes', cancelPunchOut, true);
  };

  const confirmDeleteIpo = (): void => {
    showModalDialog('Delete IPO', <div>Are you sure you want to delete the IPO?</div>, '18vw', 'No', null, 'Yes', deletePunchOut, true);
  };
    ...
    <Container>
      <HeaderContainer>
        ...
        <ButtonContainer>
          {canDelete && (
            <>
              <Button variant="outlined" color="danger" onClick={(): void => confirmDeleteIpo()}>
          ...
          {canCancel && (
            <>
              <ButtonSpacer />
              <Button variant="outlined" onClick={(): void => confirmCancelIpo()}>
          ...
          {showEditButton && (
            <>
              ...
              <Link to={`${newPathname}/EditIPO/${ipoId}`}>
                <Button disabled={!isEditable} variant="outlined">
          ...
          {isAdmin && (
            <>
              <ButtonSpacer />
              <Switch default checked={isUsingAdminRights} onChange={(): void => setIsUsingAdminRights((prevValue: boolean) => !prevValue)} label={'Admin mode'} />
            </>
          )}
        </ButtonContainer>
      </HeaderContainer>
