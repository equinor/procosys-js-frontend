...
import LibraryTreeview, { LibraryProvider } from './LibraryTreeview/LibraryTreeview';
...
const libraryTypePrefixId = ['mode_', 'journey_', 'tf_register_', 'rt_', 'field_withinput_', 'field_withoutinput_', 'header_rt_'];
...
const Library = (): JSX.Element => {
  ...
  useEffect(() => {
    ...
    const libraryV2Index = pathSegments.findIndex((segment) => segment.toLowerCase() === 'libraryv2');

    if (libraryV2Index !== -1 && pathSegments.length > libraryV2Index + 1) {
      const segmentsAfterLibraryV2 = pathSegments.slice(libraryV2Index + 1);
      const libraryType = mapSegmentsToLibraryType(segmentsAfterLibraryV2);
  ...
  const mapSegmentsToLibraryType = (segments: string[]): string => {
    ...
    switch (firstSegment) {
      ...
      case 'preservation requirements': {
        const isPresReqType = segments.some((segment) => segment.toLowerCase().startsWith('rt_'));

        const isPresReqDef = segments.some((segment) => segment.toLowerCase().startsWith('field_withinput_'));
      ...
      case 'requirements with required user input': {
        const isPresReqDef = segments.some((segment) => segment.toLowerCase().startsWith('field_withinput_'));
        return isPresReqDef ? LibraryType.PRES_REQUIREMENT_DEFINITION : LibraryType.PRES_REQUIREMENT_TYPE;
      }
      default:
