...
const LibraryItemDetails = (props: LibraryItemProps): JSX.Element => {
  switch (props.libraryType) {
    case LibraryType.TAG_FUNCTION: {
      return <TagFunction tagFunctionCode={props.tagFunctionCode} registerCode={props.registerCode} />;
    }
    case LibraryType.MODE:
      return <Mode forceUpdate={props.forceUpdate} modeId={Number(props.libraryItem)} setDirtyLibraryType={(): void => props.setDirtyLibraryType(LibraryType.MODE)} />;
    case LibraryType.PRES_JOURNEY:
      return <PreservationJourney forceUpdate={props.forceUpdate} journeyId={Number(props.libraryItem)} setDirtyLibraryType={(): void => props.setDirtyLibraryType(LibraryType.PRES_JOURNEY)} />;
    case LibraryType.PRES_REQUIREMENT:
      return <PreservationRequirements forceUpdate={props.forceUpdate} setDirtyLibraryType={(): void => props.setDirtyLibraryType(LibraryType.PRES_REQUIREMENT)} />;
    case LibraryType.PRES_REQUIREMENT_TYPE:
      return (
        <PreservationRequirementType
          ...
          cancel={(): void => {
            props.setSelectedLibraryType(LibraryType.PRES_REQUIREMENT);
          }}
        />
      );
