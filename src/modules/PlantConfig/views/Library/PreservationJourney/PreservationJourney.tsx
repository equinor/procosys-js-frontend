...
import { AutoTransferMethod, Journey, Mode, PreservationJourneyProps, Step } from './types';
...
const PreservationJourney = (props: PreservationJourneyProps): JSX.Element => {
  ...
  const [breadcrumbs, setBreadcrumbs] = useState<string>(`${baseBreadcrumb} / ${sharedJourneyBreadcrumb}`);
  ...
  useEffect(() => {
    setBreadcrumbs(`${baseBreadcrumb} / ${selectedProject?.description ?? sharedJourneyBreadcrumb}`);
  }, [selectedProject]);
  ...
  useEffect(() => {
    ...
    (async (): Promise<void> => {
      try {
        const modes = await preservationApiClient.getModes(false, (cancel: Canceler) => (requestCanceler = cancel));
  ...
  const getJourney = async (journeyId: number): Promise<void> => {
    setIsLoading(true);
    try {
      await preservationApiClient.getJourney(journeyId, true).then((response) => {
        setJourney(response);
  ...
  const updateJourney = async (): Promise<boolean> => {
    try {
      await preservationApiClient.updateJourney(newJourney.id, newJourney.title, newJourney.rowVersion, newJourney.project?.name);
      props.setDirtyLibraryType();
      return true;
    } catch (error) {
  ...
  const saveUpdatedJourney = async (): Promise<void> => {
    ...
    if (journey?.title != newJourney.title || journey?.project?.id != newJourney.project?.id) {
  ...
  return (
    <Container>
      ...
      >
        ...
        <Autocomplete
          options={projects}
          optionLabel={(p: ProjectDetails): string => `${p.name} - ${p.description}`}
                  ...
                  <ResponsibleDropdownContainer>
                    <Dropdown
                      ...
                      text={
                        responsibleSelectItem
                          ...
                          : step.responsible && step.responsible.code && step.responsible.description
                    ...
                    >
                      {filteredResponsibles.map((respItem, filtRespIndex) => {
                        return (
                          <DropdownItem key={respItem.text} onClick={(event): void => setResponsibleValue(event, index, filtRespIndex)}>
                            {respItem.text}
                          </DropdownItem>
                        );
                      })}
