...
const PreservationRequirementDefinition = (props: PreservationRequirementDefinitionProps): JSX.Element => {
        ...
        for (const reqType of requirementTypes) {
          ...
          if (reqDef) {
            try {
              ...
              const singleReqType: RequirementType = await preservationApiClient.getRequirementType(reqType.id, (cancel: Canceler) => {
                requestCancellor = cancel;
                console.log('requestCancellor assigned', requestCancellor);
              });
              ...
              if (error.response) {
                ...
                console.error('Add requirement definition failed:', error.message);
              }
              showSnackbarNotification(error.message, 5000);
            }
          }
  ...
  return (
    <Container>
      ...
      <FormHeader>Add fields to this requirement definition (optional)</FormHeader>
      <FieldsContainer>
          ...
          return (
            <React.Fragment key={`field._${index}`}>
              ...
              {field.fieldType == 'Number' && (
                <div
                  style={{
                    paddingRight: 'calc(var(--grid-unit) * 2)',
                  }}
                >
                  <Checkbox
                  ...
                  >
                    <Typography
                      style={{
                        color: newRequirementDefinition.isVoided || field.isVoided ? tokens.colors.interactive.disabled__text.rgba : '',
                      }}
                      variant="body_long"
                    >
                      Show previous value
