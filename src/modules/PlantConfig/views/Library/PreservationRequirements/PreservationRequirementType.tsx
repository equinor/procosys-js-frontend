...
const PreservationRequirementType = (props: PreservationRequirementTypeProps): JSX.Element => {
  ...
  useEffect(() => {
    ...
    if (requirementType == null && !isDirty) {
      ...
      if (isDirty && newRequirementType.code && newRequirementType.icon && newRequirementType.sortKey && newRequirementType.title && !titleError) {
  ...
  const getRequirementType = async (requirementTypeId: number): Promise<void> => {
    setIsLoading(true);
    try {
      await preservationApiClient.getRequirementType(requirementTypeId).then((response) => {
