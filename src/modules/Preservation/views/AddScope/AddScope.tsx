...
const AddScope = (): JSX.Element => {
  ...
  useEffect(() => {
    ...
    (async (): Promise<void> => {
      try {
        const data = await apiClient.getJourneys(false, (cancel: Canceler) => (requestCancellor = cancel), project.name);
        setJourneys(data);
      } catch (error) {
  ...
  useEffect(() => {
    if (addScopeMethod !== AddScopeMethod.AddTagsAutoscope) {
      ...
      (async (): Promise<void> => {
        try {
          const response = await apiClient.getRequirementTypes(false, (cancel: Canceler) => {
  ...
  const submit = async (stepId?: number, requirements?: Requirement[], remark?: string | null, storageArea?: string): Promise<void> => {
    ...
    try {
      ...
      showSnackbarNotification(error.data?.data?.errors ? Object.values(error.data.data.errors).join('\n') : getErrorMessage(error), 10000);
    }
    ...
    try {
      ...
      const filteredTags = filterOnPurchaseOrderNumber(result).map((r): TagRow => {
        return {
          ...
          tableData: {
            checked: selectedTags.findIndex((tag) => tag.tagNo === r.tagNo) > -1,
          },
        };
      });
      setScopeTableData(filteredTags);
  ...
  const removeSelectedTag = (tagNo: string): void => {
    ...
    if (selectedIndex > -1) {
      ...
      setSelectedTags(() => {
        return [...selectedTags.slice(0, selectedIndex), ...selectedTags.slice(selectedIndex + 1)];
      });
