...
const CreateDummyTag = (props: CreateDummyTagProps): JSX.Element => {
  ...
  useEffect(() => {
    ...
    (async (): Promise<void> => {
      try {
        const response = await libraryApiClient.getAreas((cancel: Canceler) => {
  ...
  useEffect(() => {
    ...
    (async (): Promise<void> => {
      try {
        const response = await procosysApiClient.getPurchaseOrders(project.name, (cancel: Canceler) => {
          requestCancellor = cancel;
        });
  ...
  const checkTagNo = async (areaType: string, discipline: string, area: string | null, po: string | null, suffix: string | null): Promise<CheckAreaTagNo> => {
    try {
      const response = await apiClient.checkAreaTagNo(project.name, areaType, discipline, area, po, suffix);
  ...
  return (
    <div>
      ...
      <TopContainer>
        <ErrorContainer>{tagNoValidationError && <Typography variant="caption">{tagNoValidationError}</Typography>}</ErrorContainer>
        <Container>
          <InputContainer>
            {isLoading && <Spinner />}
            <FormFieldSpacer>
              <SelectInput onChange={setAreaTypeForm} data={areaTypes} label={'Dummy type'} disabled={isLoading || purchaseOrderNumber ? true : false || props.duplicateTagId ? true : false}>
                {(props.areaType && props.areaType.text) || 'Select'}
              </SelectInput>
            </FormFieldSpacer>
            <FormFieldSpacer>
              <SelectInput onChange={setDisciplineForm} disabled={isLoading} data={mappedDisciplines} label={'Discipline'}>
              ...
              {props.areaType && props.areaType.value == 'PoArea' ? (
                ...
                >
                  {filteredPOs.map((POItem, index) => {
                    return (
                      <DropdownItem key={index} onClick={(event): void => changePO(event, index)}>
                        {POItem.text}
                      </DropdownItem>
                    );
                  })}
                ...
                <Dropdown
                  ...
                  Icon={
                    props.area && props.area.description ? (
                      <div id="dropdownIcon" onClick={clearArea}>
                        <EdsIcon name="close" />
                      </div>
                    ) : (
                      <KeyboardArrowDown />
                ...
                >
                  {filteredAreas.map((areaItem, index) => {
                    return (
                      <DropdownItem key={index} onClick={(event): void => changeArea(event, index)}>
                        {areaItem.text}
                      </DropdownItem>
                    );
                  })}
