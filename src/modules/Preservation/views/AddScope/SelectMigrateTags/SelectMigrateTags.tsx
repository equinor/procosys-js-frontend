...
const columns = [
  {
    ...
    filter: (rows: UseTableRowProps<TagMigrationRow>[], id: number, filterType: string): UseTableRowProps<TagMigrationRow>[] => {
      return rows.filter((row) => {
        return row.original.tagNo?.toLowerCase().indexOf(filterType.toLowerCase()) > -1;
      });
    },
  },
  {
    ...
    filter: (rows: UseTableRowProps<TagMigrationRow>[], id: number, filterType: string): UseTableRowProps<TagMigrationRow>[] => {
      return rows.filter((row) => {
        return row.original.description?.toLowerCase().indexOf(filterType.toLowerCase()) > -1;
      });
    },
  ...
  },
  {
    ...
    filter: (rows: UseTableRowProps<TagMigrationRow>[], id: number, filterType: string): UseTableRowProps<TagMigrationRow>[] => {
      return rows.filter((row) => {
        return row.original.preservationRemark?.toLowerCase().indexOf(filterType.toLowerCase()) > -1;
      });
    },
  ...
  },
  {
    ...
    Cell: (rowData: TableOptions<TagMigrationRow>): JSX.Element => {
      return <div>{getFormattedDate(rowData.row.values.nextUpcommingDueTime)}</div>;
    },
    filter: (rows: UseTableRowProps<TagMigrationRow>[], id: number, filterType: string): UseTableRowProps<TagMigrationRow>[] => {
      return rows.filter((row) => {
        return getFormattedDate(row.original.nextUpcommingDueTime)?.toLowerCase().indexOf(filterType.toLowerCase()) > -1;
      });
    },
  },
  {
    ...
    filter: (rows: UseTableRowProps<TagMigrationRow>[], id: number, filterType: string): UseTableRowProps<TagMigrationRow>[] => {
      return rows.filter((row) => {
        return getFormattedDate(row.original.startDate)?.toLowerCase().indexOf(filterType.toLowerCase()) > -1;
      });
    },
  },
  {
    ...
    filter: (rows: UseTableRowProps<TagMigrationRow>[], id: number, filterType: string): UseTableRowProps<TagMigrationRow>[] => {
      return rows.filter((row) => {
        return row.original.storageArea?.toLowerCase().indexOf(filterType.toLowerCase()) > -1;
      });
    },
  ...
  },
  {
    ...
    filter: (rows: UseTableRowProps<TagMigrationRow>[], id: number, filterType: string): UseTableRowProps<TagMigrationRow>[] => {
      return rows.filter((row) => {
        return row.original.modeCode?.toLowerCase().indexOf(filterType.toLowerCase()) > -1;
      });
    },
  },
  {
    ...
    field: 'heating',
    Cell: (rowData: TableOptions<TagMigrationRow>): JSX.Element => {
      return rowData.row.values.Heating === 'Heating' ? <CheckBox /> : <></>;
    },
  ...
  },
  {
    ...
    field: 'special',
    Cell: (rowData: TableOptions<TagMigrationRow>): JSX.Element => {
      return rowData.row.values['Special req'] !== 'No' ? <CheckBox /> : <></>;
    },
    Filter: SelectColumnFilter,
    filter: 'equals',
  },
  {
    ...
    field: 'isPreserved',
    Cell: (rowData: TableOptions<TagMigrationRow>): JSX.Element => {
      return rowData.row.values.Preserved === 'Preserved' ? <CheckBox /> : <></>;
    },
    Filter: SelectColumnFilter,
    filter: 'equals',
  },
  {
    ...
    filter: (rows: UseTableRowProps<TagMigrationRow>[], id: number, filterType: string): UseTableRowProps<TagMigrationRow>[] => {
      return rows.filter((row) => {
        return row.original.mccrResponsibleCodes?.toLowerCase().indexOf(filterType.toLowerCase()) > -1;
      });
    },
  ...
  },
  {
    ...
    filter: (rows: UseTableRowProps<TagMigrationRow>[], id: number, filterType: string): UseTableRowProps<TagMigrationRow>[] => {
      return rows.filter((row) => {
        return row.original.purchaseOrderTitle?.toLowerCase().indexOf(filterType.toLowerCase()) > -1;
      });
    },
  },
  {
    ...
    filter: (rows: UseTableRowProps<TagMigrationRow>[], id: number, filterType: string): UseTableRowProps<TagMigrationRow>[] => {
      return rows.filter((row) => {
        return row.original.commPkgNo?.toLowerCase().indexOf(filterType.toLowerCase()) > -1;
      });
    },
  },
  {
    ...
    filter: (rows: UseTableRowProps<TagMigrationRow>[], id: number, filterType: string): UseTableRowProps<TagMigrationRow>[] => {
      return rows.filter((row) => {
        return row.original.mcPkgNo?.toLowerCase().indexOf(filterType.toLowerCase()) > -1;
      });
    },
  },
  {
    ...
    filter: (rows: UseTableRowProps<TagMigrationRow>[], id: number, filterType: string): UseTableRowProps<TagMigrationRow>[] => {
      return rows.filter((row) => {
        return row.original.tagFunctionCode?.toLowerCase().indexOf(filterType.toLowerCase()) > -1;
      });
    },
  },
];
  ...
  return (
    <Container>
      <Header>
        ...
        {purchaseOrderNumber && <div style={{ marginLeft: 'calc(var(--grid-unit) * 4)' }}>PO number: {purchaseOrderNumber}</div>}
      </Header>
      <TopContainer>
        <InnerContainer>
        ...
        <ButtonsContainer>
          ...
          <ButtonSeparator />
          <Button onClick={props.removeFromMigrationScope} disabled={props.selectedTags.length === 0}>
            Remove from migration scope
          </Button>
          <ButtonSeparator />
          <Button onClick={props.nextStep} disabled={props.selectedTags.length === 0}>
            Next
          </Button>
        </ButtonsContainer>
      </TopContainer>
      ...
      <TableContainer>
        <ProcosysTable
          onSelectedChange={(rowData: TagMigrationRow[], ids: any): void => {
