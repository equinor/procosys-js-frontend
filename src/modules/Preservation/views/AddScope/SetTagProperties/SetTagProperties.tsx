...
const SetTagProperties = ({ areaType, submitForm, previousStep, journeys = [], requirementTypes = [], addScopeMethod, isLoading }: SetTagPropertiesProps): JSX.Element => {
  ...
  const hasUnsavedChanges = (): boolean => {
    return (journey && journey != -1) || (step && step != null) || (requirements && requirements.length > 0) || remark != '' || storageArea != '';
  };
      ...
      try {
        if (addScopeMethod === AddScopeMethod.AddTagsAutoscope) {
          ...
          if (requirementsMappedForApi.length > 0) {
            ...
            showSnackbarNotification('Error occurred. Requirements are not provided.', 5000);
            return;
          }
        }
        ...
        showSnackbarNotification('Error occurred during submission.', 5000);
      }
    } else {
      showSnackbarNotification('Error occurred. Step is not provided.', 5000);
    }
  };
  ...
  if (journeys.length <= 0 || (addScopeMethod !== AddScopeMethod.AddTagsAutoscope && requirementTypes.length <= 0)) {
    return (
      <div>
        ...
        <Container>
          <div>Missing journey or requirement definitions. Please create this first, and try again.</div>
          <ButtonContainer>
  ...
  return (
    <div>
      <Header>
        {addScopeMethod === AddScopeMethod.MigrateTags && <Typography variant="h1">Migrate preservation scope</Typography>}

        {addScopeMethod !== AddScopeMethod.MigrateTags && <Typography variant="h1">Add preservation scope</Typography>}
      ...
      <Container>
        <div>
          <InputContainer>
            <SelectInput maxHeight={'300px'} onChange={setJourneyFromForm} data={mappedJourneys} label={'Preservation journey for all selected tags'}>
              {(journey > -1 && journeys[journey].title) || 'Select journey'}
            </SelectInput>
          </InputContainer>
          <InputContainer>
            <SelectInput onChange={setStepFromForm} data={mappedSteps} disabled={mappedSteps.length <= 0 || areaType == 'PoArea'} label={'Preservation step'}>
        ...
        <ButtonContainer>
          ...
          <Button onClick={submit} color="primary" disabled={!formIsValid || isLoading}>
            {isLoading && (
              <CenterContent>
