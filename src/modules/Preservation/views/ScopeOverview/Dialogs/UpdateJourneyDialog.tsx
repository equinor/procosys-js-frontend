...
import { ButtonContainer, ButtonSpacer, DialogContainer, Divider, InputSpacer, MainContainer, Scrim, TableContainer, Title } from './SharedCode/Dialogs.style';
...
const UpdateJourneyDialog = ({ tags, open, onClose }: UpdateJourneyDialogProps): JSX.Element | null => {
  if (!open) {
    return null;
  }
  ...
  const [tagJourneyOrStepEdited, setTagJourneyOrStepEdited] = useState<boolean>(false);
  const [tagJourneyAndStepValid, setTagJourneyAndStepValid] = useState<boolean>(false);
  ...
  useEffect(() => {
    ...
    (async (): Promise<void> => {
      try {
        const data = await apiClient.getJourneys(true, (cancel: Canceler) => (requestCancellor = cancel), project.name);
        setJourneys(data);
      } catch (error) {
  ...
  const setSelectedStep = (stepId: number, journey?: Journey): void => {
    if (journey) {
      let selectedStep: Step | undefined = journey.steps.find((pStep: Step) => pStep.id === stepId);
      if (!selectedStep) {
  ...
  useEffect(() => {
    ...
    if (journeys.length > 0 && journey) {
      ...
      if (poTag) {
        const stepForSupplier = journey.steps.find((step) => step.mode.forSupplier);
        if (!stepForSupplier) {
  ...
  const updateTagJourneyAndStep = async (): Promise<void> => {
    try {
      ...
      const errorMessageConsole = 'Error updating journey, step, requirements, or description';
      if (error instanceof ProCoSysApiError) {
        handleErrorFromBackend(error, errorMessageConsole, setValidationErrorMessage);
      } else {
    ...
    <Scrim>
      <DialogContainer width={'80vw'}>
        ...
        <Content>
          ...
          <InputSpacer>
            <SelectInput maxHeight={'300px'} onChange={setJourneyFromForm} data={mappedJourneys} label={'Preservation journey for selected tag'}>
              {journey ? journey.title : 'Select journey'}
            </SelectInput>
          </InputSpacer>
          <InputSpacer>
            <SelectInput onChange={(stepId): void => setSelectedStep(stepId, journey)} data={mappedSteps} disabled={poTag || isNewJourney} label={'Preservation step'}>
              {(step && step.title) || 'Select step'}
            </SelectInput>
            {poTag && <Typography variant="meta">At least one purchase order tag has been chosen. Step must therefore be supplier step and will be set automatically.</Typography>}
          </InputSpacer>
          <MainContainer style={{ height: '100%' }}>
            {nonUpdateableTags.length > 0 && (
              <TableContainer isHalfSize={updateableTags.length > 0}>
                ...
                <DialogTable tags={nonUpdateableTags} columns={columns} toolbarText="journey for tag(s) will not be updated" toolbarColor={tokens.colors.interactive.danger__text.rgba} />
              </TableContainer>
            )}
            {updateableTags.length > 0 && (
              <TableContainer isHalfSize={nonUpdateableTags.length > 0}>
                <DialogTable tags={updateableTags} columns={columns} toolbarText="journey for tag(s) will be updated" toolbarColor={tokens.colors.interactive.primary__resting.rgba} />
              </TableContainer>
            )}
          </MainContainer>
        </Content>
        <ButtonContainer>
          ...
          <ButtonSpacer />
          <Button onClick={save} color="primary" disabled={!tagJourneyAndStepValid}>
            Save
          </Button>
          {showSpinner && <Spinner />}
        </ButtonContainer>
