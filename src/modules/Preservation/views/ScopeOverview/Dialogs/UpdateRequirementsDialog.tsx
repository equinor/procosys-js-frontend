...
import { ButtonContainer, ButtonSpacer, Content, DialogContainer, Divider, Header, InputSpacer, Scrim, Title } from './SharedCode/Dialogs.style';
...
const UpdateRequirementsDialog = ({ open, onClose, tagId }: UpdateRequirementsDialogProps): JSX.Element | null => {
  if (!open) {
    return null;
  }
  ...
  const [requirementsOrDescriptionEdited, setRequirementsOrDescriptionEdited] = useState<boolean>(false);
  ...
  useEffect(() => {
    ...
    (async (): Promise<void> => {
      if (tagId) {
        try {
          const details = await apiClient.getTagDetails(tagId, (cancel: Canceler) => (requestCancellor = cancel));
          setTag(details);
  ...
  useEffect(() => {
    ...
    (async (): Promise<void> => {
      if (tagId) {
        try {
          const response = await apiClient.getTagRequirements(tagId, true, true, (cancel: Canceler) => (requestCancellor = cancel));
   ...
   */
  useEffect(() => {
    if (tag && (JSON.stringify(requirements) != JSON.stringify(originalRequirements) || (description && description != tag.description))) {
  ...
  const updateRemarkAndStorageArea = async (): Promise<string> => {
    try {
      ...
      if (error instanceof ProCoSysApiError) {
        handleErrorFromBackend(error, 'Error updating remark and storage area', setValidationErrorMessage);
        throw error.message;
      }
  ...
  const updateRequirementsAndDescription = async (currentRowVersion: string): Promise<void> => {
    try {
      if (tag && description) {
        ...
        await apiClient.updateTagRequirements(tag.id, description, currentRowVersion, updatedRequirements, newRequirements, deletedRequirements);
      }
    } catch (error) {
      if (error instanceof ProCoSysApiError) {
        handleErrorFromBackend(error, 'Error updating requirements or description', setValidationErrorMessage);
      } else {
        console.error('Error updating requirements or description', 'An unknown error occurred', error);
    ...
    <Scrim>
      ...
      >
        ...
        <Content style={{ height: 'auto' }}>
          ...
          <InputSpacer style={{ maxWidth: '480px' }}>
            <TextField
              ...
              disabled={tag && tag.tagType && !dummyTagTypes.includes(tag.tagType)}
          ...
          <InputSpacer style={{ maxWidth: '150px' }}>
            <TextField id={'StorageArea'} label="Storage area" defaultValue={tag ? tag.storageArea : ''} inputRef={storageAreaInputRef} placeholder="Write here" meta="Optional" onChange={remarkOrStorageAreaChange} />
          </InputSpacer>
        ...
        <ButtonContainer style={{ marginTop: 'auto' }}>
          ...
          <ButtonSpacer />
          <Button onClick={save} color="primary" disabled={!remarkOrStorageAreaEdited && !requirementsOrDescriptionEdited}>
