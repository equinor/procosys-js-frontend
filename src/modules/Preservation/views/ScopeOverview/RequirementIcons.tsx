  ...
  return (
    <RequirementsContainer>
      {tag.requirements.map((req) => {
        return (
          <RequirementIcon key={req.id} isDue={isRequirementDue(req) || isRequirementOverdue(req)} isReadyToBePreserved={req.readyToBePreserved}>
            <Tooltip title={req.requirementTypeCode} arrow={true} enterDelay={200} enterNextDelay={100}>
              <span>
                <PreservationIcon variant={req.requirementTypeIcon} />
              </span>
            </Tooltip>
          </RequirementIcon>
        );
