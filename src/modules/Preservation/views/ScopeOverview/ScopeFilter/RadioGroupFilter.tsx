...
import { CollapseInfo, Collapse, ExpandedContainer, ScopeFilterRadio } from './ScopeFilter.style';
...
const RadioGroupFilter = ({ options, value, onChange, label = '', icon }: RadioGroupFilterProps): JSX.Element => {
  ...
  const onSelectionChanged = (e: React.ChangeEvent<HTMLInputElement>): void => {
      ...
      {isExpanded && (
        <ExpandedContainer>
          <UnstyledList>
            {options.map((option) => (
              <li key={option.value}>
                <ScopeFilterRadio label={option.title} value={option.value} checked={(!value && option.default) || value === option.value} onChange={onSelectionChanged} />
              </li>
            ))}
          </UnstyledList>
        </ExpandedContainer>
