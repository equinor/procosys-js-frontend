...
const SavedFilters = (props: SavedFiltersProps): JSX.Element => {
    ...
    const { project, apiClient } = usePreservationContext();

    //Set selected filter to null, if filter values are changed
    useEffect((): void => {
        if (props.savedTagListFilters && props.selectedSavedFilterTitle) {
            const selectedFilterIndex = props.savedTagListFilters.findIndex(
                (filter) => filter.title == props.selectedSavedFilterTitle
            );
            setSelectedFilterIndex(selectedFilterIndex);
            if (
                props.selectedSavedFilterTitle &&
                JSON.stringify(props.tagListFilter) !=
                    props.savedTagListFilters[selectedFilterIndex].criteria
            ) {
    ...
    const onSaveFilter = async (): Promise<void> => {
        try {
            await apiClient.addSavedTagListFilter(
                project.name,
                newFilterTitle,
                newFilterIsDefault,
                JSON.stringify(props.tagListFilter)
    ...
    const onDeleteFilter = async (index: number): Promise<void> => {
        try {
            const filter =
                props.savedTagListFilters && props.savedTagListFilters[index];
            if (filter) {
                await apiClient.deleteSavedTagListFilter(
                    filter.id,
                    filter.rowVersion
                );
                props.refreshSavedTagListFilters();
    ...
    const updateSavedFilter = async (
        filter: SavedTagListFilter
    ): Promise<void> => {
        try {
            await apiClient.updateSavedTagListFilter(
                filter.id,
                filter.title,
    ...
    const onSelectFilter = (index: number): void => {
        if (props.savedTagListFilters) {
            props.setSelectedSavedFilterTitle(
                props.savedTagListFilters[index].title
            );
            props.setTagListFilter(
                JSON.parse(props.savedTagListFilters[index].criteria)
            );
    ...
    return (
        <Container>
            ...
            <ListContainer>
                {props.savedTagListFilters &&
                    props.savedTagListFilters.map((filter, index) => {
                        return (
