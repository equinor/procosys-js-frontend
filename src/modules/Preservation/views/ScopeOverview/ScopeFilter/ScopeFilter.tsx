...
import { BookmarksOutlined, Close, KeyboardArrowDown, KeyboardArrowUp } from '@mui/icons-material';
...
export type TagListFilterParamType = 'modeIds' | 'journeyIds' | 'dueFilters' | 'requirementTypeIds' | 'tagFunctionCodes' | 'disciplineCodes' | 'preservationStatus' | 'actionStatus';
...
const ScopeFilter = ({
  ...
  const { project, apiClient, purchaseOrderNumber: purchaseOrderNumber, setCurrentPurchaseOrderNumber } = usePreservationContext();
    ...
    <Container>
      <Header filterActive={filterActive}>
        ...
        <div style={{ display: 'flex' }}>
          <OptionsDropdown icon="microsoft_excel" variant="ghost" closeOnClick={true}>
            <DropdownItem
              onClick={(): void => {
            ...
            <Tooltip
              ...
              disableHoverListener={numberOfTags && numberOfTags < 100 ? true : false}
              style={{ textAlign: 'center' }}
            >
              <div>
                <DropdownItem
                  disabled={numberOfTags && numberOfTags > 100 ? true : false}
                  onClick={(): void => {
      ...
      <CheckboxFilter
        ...
        icon={'notifications'}
      />
      <RadioGroupFilter options={VOIDED} onChange={onVoidedFilterChanged} value={localTagListFilter.voidedFilter} label="Voided/unvoided tags" icon={'delete_forever'} />
      ...
      <MultiSelectFilter headerLabel="Responsible" items={responsibles} onChange={responsibleFilterUpdated} selectedItems={localTagListFilter.responsibleIds} inputLabel="Responsible" icon={<EdsIcon name="person" />} />
      <MultiSelectFilter headerLabel="Area (on-site)" items={areas} onChange={areaFilterUpdated} selectedItems={localTagListFilter.areaCodes} inputLabel="Area" icon={<AreaIcon />} />
    </Container>
  );
};
