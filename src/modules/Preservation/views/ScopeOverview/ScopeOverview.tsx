...
import { PreservedTag, PreservedTags, SavedTagListFilter, TagListFilter } from './types';
...
import { getCachedFilter, setCachedFilter, deleteCachedFilter } from './preservationHelpers';
...
const ScopeOverview: React.FC = (): JSX.Element => {
  const { project, availableProjects, setCurrentProject, apiClient, purchaseOrderNumber: purchaseOrderNumber, setCurrentPurchaseOrderNumber: setCurrentPurchaseOrderNumber } = usePreservationContext();
  ...
  const [inServiceTagsSelected, setInServiceTagsSelected] = useState<boolean>();
  ...
  const [unStartableTagsSelected, setUnstartableTagsSelected] = useState<boolean>();
  ...
  const [showEditTagJourneyDialog, setShowEditTagJourneyDialog] = useState<boolean>(false);
  const [showEditRequirementsDialog, setShowEditRequirementsDialog] = useState<boolean>(false);
  ...
  const [nonTransferableTags, setNonTransferableTags] = useState<PreservedTag[]>([]);
  ...
  useEffect(() => {
    ...
    setFilteredProjects(
      availableProjects.filter((p: ProjectDetails) => {
        return p.name.toLowerCase().indexOf(filterForProjects.toLowerCase()) > -1 || p.description.toLowerCase().indexOf(filterForProjects.toLowerCase()) > -1;
      })
    );
  }, [filterForProjects]);
  ...
  useEffect(() => {
    setInServiceTagsSelected(selectedTags.find((t) => t.readyToBeSetInService) ? true : false);
    ...
    setUnstartableTagsSelected(selectedTags.find((t) => t.readyToUndoStarted && !t.isVoided) ? true : false);
    setEditableTagSelected(selectedTags.find((t) => t.readyToBeEdited && !t.isVoided) ? true : false);
    ...
    if (savedTagListFilters) {
      ...
      try {
        ...
        return await apiClient
          .getPreservedTags(project.name, page, pageSize, orderBy, orderDirection, tagListFilter, (c) => {
  ...
  const exportTagsToExcel = async (withHistory: boolean): Promise<void> => {
    try {
      ...
      await apiClient.exportTagsWithHistoryToExcel(project.name, orderByField, orderDirection, tagListFilter, withHistory).then((response) => {
  ...
  const setInService = async (tags: PreservedTag[]): Promise<void> => {
    try {
      ...
      refreshScopeList();
      refreshFilterValues();
      showSnackbarNotification(`${tags.length} tag(s) have been successfully set in service.`);
    } catch (error) {
  ...
  const transfer = async (tagsToTransfer: PreservedTag[]): Promise<void> => {
    try {
      ...
      refreshScopeList();
      refreshFilterValues();
      showSnackbarNotification(`${tagsToTransfer.length} tag(s) have been successfully transferred.`);
    } catch (error) {
      console.error('Transfer failed: ', error.message, error.data);
      showSnackbarNotification(error.message);
    }
  ...
  const transferDialog = (): void => {
    ...
    const transferButton = newTransferableTags.length > 0 ? 'Transfer' : null;
    ...
    showModalDialog(
      'Transferring',
      <Router>
        <TransferDialog transferableTags={newTransferableTags} nonTransferableTags={newNonTransferableTags} />
      </Router>,
      '80vw',
      backToListButton,
      null,
      transferButton,
      newTransferableTags.length > 0 ? () => transfer(newTransferableTags) : null
    );
  };
  ...
  const unStartPreservation = async (): Promise<void> => {
    try {
      ...
      refreshScopeList();
      refreshFilterValues();
      showSnackbarNotification("Status was set to 'Not started' for selected tag(s).");
    } catch (error) {
      console.error('Undo start preservation failed: ', error.message, error.data);
      showSnackbarNotification(error.message);
    }
    return Promise.resolve();
  };
  ...
  const undoStartPreservationDialog = (): void => {
    ...
    const undoStartButton = unstartableTags.length > 0 ? 'Undo "start preservation"' : null;
    const undoStartFunc = unstartableTags.length > 0 ? unStartPreservation : null;

    showModalDialog(
      'Undo "start preservation"',
      <Router>
        <UndoStartPreservationDialog unstartableTags={unstartableTags} nonUnstartableTags={nonUnstartableTags} />
      </Router>,
      '80vw',
      backToListButton,
      null,
      ...
      <ContentContainer withSidePanel={displayFilter}>
        <HeaderContainer ref={moduleHeaderContainerRef}>
          <LeftPartOfHeader>
            <Header>
              ...
              <StyledButton className="showOnlyOnTablet" variant="ghost" onClick={(): void => setShowActions(!showActions)}>
                {!showActions && (
                  <>
            ...
            <ActionsContainer showActions={showActions}>
              <Dropdown disabled={project.id === -1} maxHeight="300px" text={project.name} onFilter={setFilterForProjects}>
                {filteredProjects.map((projectItem, index) => {
                  return (
                    <DropdownItem key={index} onClick={(event): void => changeProject(event, index)}>
              ...
              {purchaseOrderNumber && (
                <div
                  style={{
                    ...
                    marginRight: 'calc(var(--grid-unit) * 4)',
                  }}
                >
              ...
              <Tooltip title={!permissions.includes('PRESERVATION_PLAN/CREATE') ? 'You do not have permission to add scope' : ''}>
                <div>
                  <Dropdown disabled={project.id === -1 || !permissions.includes('PRESERVATION_PLAN/CREATE')} text="Add scope">
                    <Link to={`AddScope/selectTagsManual`}>
                      <DropdownItem>Add tags manually</DropdownItem>
                    </Link>
                    <Link to={'AddScope/selectTagsAutoscope'}>
                      <DropdownItem>Autoscope by tag function</DropdownItem>
                    </Link>
                    <Link to={'AddScope/createDummyTag'}>
                      <DropdownItem>Create dummy tag</DropdownItem>
                    </Link>
                    <Link to={duplicatableTagSelected ? 'AddScope/duplicateDummyTag/' + (selectedTags.length == 1 ? selectedTags[0].id.toString() : '') : ''}>
                      <DropdownItem disabled={!duplicatableTagSelected}>Duplicate dummy tag</DropdownItem>
                    </Link>
                    <Link to={'AddScope/selectMigrateTags'}>
                      <DropdownItem>Migrate tags from old (temporary)</DropdownItem>
                    </Link>
                  </Dropdown>
                </div>
              </Tooltip>
              ...
              <IconBar className="showOnlyOnTablet">
                <Button onClick={preservedDialog} disabled={!preservableTagsSelected}>
                  Preserved this week
                </Button>
              </IconBar>
            </ActionsContainer>
          ...
          <ActionsContainer showActions={showActions}>
            <IconBar>
              <StyledButton className="hideOnTablet" onClick={preservedDialog} disabled={!preservableTagsSelected}>
                Preserved this week
              </StyledButton>
              <StyledButton variant="ghost" title="Start preservation for selected tag(s)" onClick={startPreservationDialog} disabled={!startableTagsSelected}>
                <EdsIcon name="play" color={!startableTagsSelected ? tokens.colors.interactive.disabled__border.rgba : ''} />
                Start
              </StyledButton>
              <StyledButton variant="ghost" title="Transfer selected tag(s)" onClick={transferDialog} disabled={!transferableTagsSelected}>
                <EdsIcon name="fast_forward" color={!transferableTagsSelected ? tokens.colors.interactive.disabled__border.rgba : ''} />
                Transfer
              </StyledButton>
              <StyledButton variant="ghost" title="Complete selected tag(s)" onClick={showCompleteDialog} disabled={!completableTagsSelected}>
                <EdsIcon name="done_all" color={!completableTagsSelected ? tokens.colors.interactive.disabled__border.rgba : ''} />
                Complete
              </StyledButton>
              <OptionsDropdown text="More options" icon="more_vertical" variant="ghost">
                <DropdownItem disabled={selectedTags.length != 1 || !editableTagSelected} onClick={(): void => setShowEditRequirementsDialog(true)}>
                  <EdsIcon name="edit_text" color={selectedTags.length != 1 || !editableTagSelected ? tokens.colors.interactive.disabled__border.rgba : tokens.colors.text.static_icons__tertiary.rgba} />
                  Update Requirements
                </DropdownItem>
                <DropdownItem disabled={!editableTagSelected} onClick={(): void => setShowEditTagJourneyDialog(true)}>
                  <EdsIcon name="edit_text" color={!editableTagSelected ? tokens.colors.interactive.disabled__border.rgba : tokens.colors.text.static_icons__tertiary.rgba} />
                  Update Journey
                </DropdownItem>
                <DropdownItem disabled={selectedTags.length === 0} onClick={(): void => setShowTagRescheduleDialog(true)}>
                ...
                <DropdownItem disabled={!voidedTagsSelected} onClick={(): void => showRemoveDialog()}>
                ...
                <DropdownItem disabled={!unvoidedTagsSelected} onClick={(): void => showVoidDialog(true)}>
                ...
                <DropdownItem disabled={!voidedTagsSelected} onClick={(): void => showVoidDialog(false)}>
                ...
                <DropdownItem disabled={!unStartableTagsSelected} onClick={undoStartPreservationDialog}>
                  <EdsIcon name="edit_text" color={!unStartableTagsSelected ? tokens.colors.interactive.disabled__border.rgba : tokens.colors.text.static_icons__tertiary.rgba} />
                  Undo start
                </DropdownItem>
              </OptionsDropdown>
              <Tooltip
                title={
                  <TooltipText>
                    ...
                    <p>Filter result {numberOfTags} items</p>
                  </TooltipText>
                }
              ...
              {!purchaseOrderNumber && (
                <Typography variant="caption">
                  <Tooltip title="To work on preservation scope not yet migrated." enterDelay={200} enterNextDelay={100} arrow={true}>
                    <Button variant="ghost" onClick={navigateToOldPreservation}>
        ...
        {displayFlyout && (
          <Flyout close={closeFlyout}>
            <TagFlyout tagId={flyoutTagId} close={closeFlyout} setDirty={(): void => setScopeIsDirty(true)} />
          </Flyout>
        )}
      </ContentContainer>
