import { Container, SingleIconContainer, TagLink, TagStatusLabel, TableRow, ReqIcon } from '@procosys/modules/Preservation/views/ScopeOverview/ScopeOverviewTable.style';
...
import { getFirstUpcomingRequirement, isTagOverdue } from './preservationHelpers';
  ...
  const getRequirementColumn = useMemo(
    () =>
      (row: TableOptions<PreservedTag>): JSX.Element => {
        ...
        return (
          <ReqIcon isOverdue={isTagOverdue(tag)} tag={tag} onClick={(): void => props.showTagDetails(tag)}>
  ...
  const getDueColumn = useMemo(
    () =>
      (row: TableOptions<PreservedTag>): JSX.Element => {
        ...
        const tag = row.value as PreservedTag;
        return (
          <TableRow isOverdue={requirement && isTagOverdue(tag) ? true : false} tag={tag}>
      ...
      (row: TableOptions<PreservedTag>): JSX.Element => {
        ...
        return (
          <TagLink isOverdue={isTagOverdue(tag)} tag={tag}>
            <Tooltip title={tag.actionStatus === ActionStatus.OverDue ? 'Overdue action(s)' : 'Open action(s)'} arrow={true} enterDelay={200} enterNextDelay={100}>
              <SingleIconContainer>
                <div>
                  <EdsIcon name="notifications" size={24} color={tag.actionStatus === ActionStatus.OverDue ? tokens.colors.interactive.danger__text.rgba : tokens.colors.text.static_icons__tertiary.rgba} />
                </div>
              </SingleIconContainer>
            </Tooltip>
          </TagLink>
  ...
  const getTagNoColumn = useMemo(
    () =>
      (row: TableOptions<PreservedTag>): JSX.Element => {
        const tag = row.value as PreservedTag;
        return (
          <TagLink isOverdue={isTagOverdue(tag)} tag={tag} onClick={(): void => props.showTagDetails(tag)}>
  ...
  const getPOColumn = useMemo(
    () =>
      (row: TableOptions<PreservedTag>): JSX.Element => {
        ...
        return (
          <TableRow isOverdue={isTagOverdue(tag)} tag={tag}>
            <Tooltip title={tag.calloffNo ? `${tag.purchaseOrderNo}/${tag.calloffNo}` : tag.purchaseOrderNo ? tag.purchaseOrderNo : ''} arrow={true} enterDelay={200} enterNextDelay={100}>
              <div>{tag.calloffNo ? `${tag.purchaseOrderNo}/${tag.calloffNo}` : tag.purchaseOrderNo}</div>
            </Tooltip>
          </TableRow>
        );
      },
