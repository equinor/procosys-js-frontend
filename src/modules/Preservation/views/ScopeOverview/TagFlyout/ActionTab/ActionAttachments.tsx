...
const ActionAttachments = ({ tagId, isVoided, actionId, enableActions }: ActionAttachmentsProps): JSX.Element => {
  const { apiClient } = usePreservationContext();
  const [attachments, setAttachments] = useState<Attachment[]>([]);
  const [isLoading, setIsLoading] = useState<boolean>(false);

  const addDownloadUriToAttachment = async (attachment: Attachment): Promise<Attachment> => {
    if (!attachment.id) return attachment;

    try {
      attachment.downloadUri = await apiClient.getDownloadUrlForActionAttachment(tagId, actionId, attachment.id);
      return attachment;
    } catch (error) {
      console.error('Unable to get download URL for attachment:', error.message, error.data);
      return attachment;
    }
  };
      ...
      try {
        if (tagId != null) {
          ...
          for (const attachment of attachments) {
            const attachmentWithUri = await addDownloadUriToAttachment(attachment);
  ...
  return (
    <Container>
      <AttachmentList attachments={attachments} disabled={isVoided} addAttachments={enableActions ? addAttachments : undefined} deleteAttachment={enableActions ? deleteAttachment : undefined} />
    </Container>
  );
};
