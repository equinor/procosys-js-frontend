...
const ActionTab = ({ tagId, isVoided, setDirty }: ActionTabProps): JSX.Element => {
  ...
  return (
    <>
      ...
      {!showCreateAction && (
        <Container>
          <AddActionContainer>
            <StyledButton disabled={isVoided} variant="ghost" onClick={(): void => setShowCreateAction(true)}>
              {addIcon} Add action
            </StyledButton>
          </AddActionContainer>

          <ActionList>{actions.map((action) => createActionSection(action))}</ActionList>
        </Container>
      )}
    </>
  );
