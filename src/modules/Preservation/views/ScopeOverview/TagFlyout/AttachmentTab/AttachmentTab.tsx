...
const AttachmentTab = ({ tagId, isVoided }: AttachmentTabProps): JSX.Element => {
  ...
  const addDownloadUriToAttachment = async (attachment: Attachment): Promise<Attachment> => {
    ...
    try {
      const uri = await apiClient.getDownloadUrlForTagAttachment(tagId, attachment.id);
  ...
  const addAttachments = async (files: FileList): Promise<void> => {
    ...
    Array.from(files).forEach(async (file) => {
      try {
        ...
        const errorMessage = error.data.data.errors[''][0] || error.message;
  ...
  return (
    <Container>
      <AttachmentList attachments={attachments} disabled={isVoided} addAttachments={addAttachments} deleteAttachment={deleteAttachment} />
    </Container>
  );
};
