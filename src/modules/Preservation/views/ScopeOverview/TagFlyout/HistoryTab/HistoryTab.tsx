...
const HistoryTab = ({ tagId }: HistoryTabProps): JSX.Element => {
  ...
  const getDateColumn = (row: TableOptions<HistoryLogItem>): JSX.Element => {
    ...
    return <DueContainer isOverdue={historyItem.dueWeeks < 0}>{getFormattedDate(historyItem.createdAtUtc)}</DueContainer>;
  };
  ...
  const getUserColumn = (row: TableOptions<HistoryLogItem>): JSX.Element => {
    ...
    return (
      <Tooltip title={`${historyItem.createdBy.firstName} ${historyItem.createdBy.lastName}`} arrow={true} enterDelay={200} enterNextDelay={100}>
        <OverflowColumn>{`${historyItem.createdBy.firstName} ${historyItem.createdBy.lastName}`}</OverflowColumn>
      </Tooltip>
    );
  };
  ...
  const getDueColumn = (row: TableOptions<HistoryLogItem>): JSX.Element => {
    const historyItem = row.value as HistoryLogItem;
    return <DueContainer isOverdue={historyItem.dueWeeks < 0}>{historyItem.dueWeeks}</DueContainer>;
  };
    ...
    <>
      ...
      {showRequirementDialog && (
        <HistoryDetails close={(): void => closeHistoryDetails()}>
          {selectedHistoryItem && (
            <PreservedRequirement tagId={tagId} tagRequirementId={selectedHistoryItem.tagRequirementId} preservationRecordGuid={selectedHistoryItem.preservationRecordGuid} close={closeHistoryDetails} />
          )}
        </HistoryDetails>
      )}
    </>
