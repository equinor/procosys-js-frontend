...
const PreservedRequirement = ({ tagId, tagRequirementId, preservationRecordGuid, close }: PreservedRequirementProps): JSX.Element => {
  ...
  const getPreservationRecord = (): Canceler | null => {
    ...
    (async (): Promise<void> => {
      try {
        ...
        const preservationRecord = await apiClient.getPreservationRecord(tagId, tagRequirementId, preservationRecordGuid, (cancel: Canceler) => (requestCancellor = cancel));
  ...
  const getRequirementField = (field: TagRequirementField): JSX.Element => {
    switch (field.fieldType.toLowerCase()) {
      ...
      case 'attachment':
        return (
          <div>
            <AttachmentLink onClick={downloadAttachment}>{field.currentValue && field.currentValue.fileName}</AttachmentLink>
            {field.label}
          </div>
        );
      default:
    ...
    <div>
      <Section>
        <div style={{ display: 'flex', alignItems: 'center' }}>
          <Typography variant="h4">{preservationRecord.requirementType.title}</Typography>
        ...
        <Typography variant="h6">{preservationRecord.requirementDefinition.title}</Typography>
        <div
          style={{
            display: 'flex',
            alignItems: 'baseline',
      ...
      <Section>
        {preservationRecord.fields.map((field) => {
          return <Field key={field.id}>{getRequirementField(field)}</Field>;
        })}
      </Section>
      <Section>
