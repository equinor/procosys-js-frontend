...
const Requirements = ({
    ...
    const hasUnsavedChangesForRequirement = (
    ...
    const setNumberFieldValue = (
        requirementId: number,
        fieldId: number,
        value: string
    ): void => {
        const newRequirementValues = [...requirementValues];
        const requirement = newRequirementValues.find(
        ...
        const numberFieldValue =
            numberFieldIsNA || value === '' ? null : Number(value); // invalid numbers become "NaN" (validated at save)

        if (requirement) {
            const fieldIndex = requirement.numberValues.findIndex(
    ...
    const setCheckBoxFieldValue = (
        ...
        if (requirement) {
            const fieldIndex = requirement.checkBoxValues.findIndex(
                (field) => field.fieldId == fieldId
            );
            if (fieldIndex > -1) {
    ...
    const saveRequirement = (requirementId: number): void => {
        const requirement = requirementValues.find(
            (req) => req.requirementId == requirementId
        );

        if (!requirement) {
            console.error(
                `No values to record found for requirementId ${requirementId}`
            );
            return;
        }
                    ...
                    <Container key={requirement.id}>
                        <Section>
                            ...
                            >
                                ...
                                >{`${requirement.intervalWeeks} weeks`}</Typography>
                                <Typography
                                    variant="caption"
                                    style={{
                                        marginLeft:
                        ...
                        <Section>
                            ...
                            >
                                <Button
                                    disabled={
                                        !hasUnsavedChangesForRequirement(
                                            requirement.id
                                        )
                                    }
                                    onClick={(): void =>
                                        saveRequirement(requirement.id)
                                    }
                                >
                                    Save
                                </Button>
                                <Button
                                    disabled={
                                        !isPreserveButtonEnabled(
                                            requirement.id,
                                            requirement.readyToBePreserved
                                        )
                                    }
                                    onClick={(): void =>
                                        preserveRequirement(requirement.id)
                                    }
                                    style={{
                                        marginLeft:
                                            'calc(var(--grid-unit) * 2)',
