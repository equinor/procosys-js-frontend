...
const Requirements = ({ tagId, requirements, readonly, recordTagRequirementValues, preserveRequirement, refreshRequirements }: RequirementProps): JSX.Element => {
  ...
  const getRequirementField = (requirementId: number, field: TagRequirementField): JSX.Element => {
    switch (field.fieldType.toLowerCase()) {
      ...
      case 'checkbox':
        return <RequirementCheckboxField requirementId={requirementId} field={field} readonly={readonly} isChecked={getCheckboxValue(requirementId, field)} onFieldChange={setCheckBoxFieldValue} />;
      case 'number':
        return <RequirementNumberField requirementId={requirementId} field={field} readonly={readonly} onFieldChange={setNumberFieldValue} />;
      case 'attachment':
        return <RequirementAttachmentField requirementId={requirementId} field={field} tagId={tagId} readonly={readonly} onAttachmentUpdated={refreshRequirements} />;
        ...
        return (
          <Container key={requirement.id}>
            <Section>
              ...
              >
                <Typography variant="h4">{requirement.requirementType.title}</Typography>
                <div
                  style={{
              ...
              <Typography variant="h6">{requirement.requirementDefinition.title}</Typography>
              <div
                style={{
                  display: 'flex',
                  flexWrap: 'wrap',
              ...
              >
                ...
                <Typography variant="body_short" bold style={{ marginLeft: 'var(--grid-unit)' }}>
                  <NextInfo isOverdue={isOverdue}>{requirement.nextDueAsYearAndWeek}</NextInfo>
                </Typography>
                <Typography
                  variant="caption"
                  style={{
                ...
                <Typography variant="body_short" bold style={{ marginLeft: 'var(--grid-unit)' }}>
                  <NextInfo isOverdue={isOverdue}>{requirement.nextDueWeeks}</NextInfo>
                </Typography>
              </div>
            </Section>
            <Section>
              {requirement.fields.map((field) => {
                return <Field key={field.id}>{getRequirementField(requirement.id, field)}</Field>;
              })}
            </Section>
            <Section>
              <TextField
            ...
            <Section>
              ...
              >
                <Button disabled={!hasUnsavedChangesForRequirement(requirement.id)} onClick={(): void => saveRequirement(requirement.id)}>
                  Save
                </Button>
                <Button
