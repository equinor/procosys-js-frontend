import { Container, Header, HeaderActions, HeaderNotification, NotificationIcon, StatusLabel, StatusText, StyledButton, Tabs, TagNoContainer, TagsGrid } from './TagFlyout.style';
...
import { NotificationsOutlined, PlayArrowOutlined, Close } from '@mui/icons-material';
    ...
    <Container>
      ...
      <Header>
        {isDeletedInSource ? (
          <TagNoContainer isStandardTag={false}>
            <Typography variant="h1"> {tagDetails ? tagDetails.tagNo : '-'}</Typography>
          </TagNoContainer>
        ) : (
        ...
        <HeaderActions>
          {!isVoided && preservationIsStarted && (
            <StyledButton disabled={!isPreserveTagButtonEnabled()} onClick={preserveTag}>
              {isPreservingTag && (
                <span
                  style={{
                    display: 'flex',
          ...
          {!isVoided && preservationIsNotStarted && (
            <StyledButton variant="ghost" title="Start preservation" disabled={isStartingPreservation} onClick={startPreservation}>
              <PlayArrowOutlined />
            </StyledButton>
          )}
      ...
      <TagsGrid>
        <StatusLabel status={isVoided ? 'Voided' : tagDetails && tagDetails.status}>
          <StatusText>{isVoided ? 'Voided' : tagDetails && tagDetails.status}</StatusText>
        </StatusLabel>
        {isVoidedInSource && (
          <StatusLabel status={'Voided in source'}>
            <StatusText>{isVoidedInSource ? 'Voided in source' : ''}</StatusText>
          </StatusLabel>
        )}
        {isDeletedInSource && (
          <StatusLabel status={isDeletedInSource ? 'Deleted in source' : ''}>
            <StatusText>{isDeletedInSource ? 'Deleted in source' : ''}</StatusText>
          </StatusLabel>
        )}
      </TagsGrid>
      <Tabs>
        <a className={activeTab === 'preservation' ? 'active' : 'preservation'} onClick={(): void => setActiveTab('preservation')}>
          Preservation
        </a>
        <a className={activeTab === 'actions' ? 'active' : 'actions'} onClick={(): void => setActiveTab('actions')}>
          Preservation actions
        </a>
        <a className={activeTab === 'attachments' ? 'active' : 'attachments'} onClick={(): void => setActiveTab('attachments')}>
          Attachments
        </a>
        <a className={activeTab === 'history' ? 'active' : 'history'} onClick={(): void => setActiveTab('history')}>
          History
        </a>
      </Tabs>
      {getTabContent()}
