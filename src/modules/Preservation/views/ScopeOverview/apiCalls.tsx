...
interface UpdateSavedTagListFiltersProps {
  ...
  project: { name: string };
  apiClient: {
    getSavedTagListFilters: (projectName: string) => Promise<SavedTagListFilter[]>;
  };
  showSnackbarNotification: (message: string, duration: number) => void;
}

export const updateSavedTagListFilters = async ({ setIsLoading, setSavedTagListFilters, project, apiClient, showSnackbarNotification }: UpdateSavedTagListFiltersProps): Promise<void> => {
  setIsLoading(true);
  try {
