...
export function getCachedFilter(projectId: number): TagListFilter | null {
  try {
    ...
    showSnackbarNotification('An error occurred retrieving default filter values');
    console.error('Error while retrieving cached filter values: ', error);
  }
  return null;
}

export function setCachedFilter(projectId: number, filter: TagListFilter): void {
  try {
    ScopeOverviewCache.setCache(projectId + '-filter', filter);
  } catch (error) {
    showSnackbarNotification('An error occurred when saving default filter values');
