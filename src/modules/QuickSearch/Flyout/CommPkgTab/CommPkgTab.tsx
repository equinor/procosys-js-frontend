import { Container, StyledCard100, StyledCard50, StyledTypographyLink, StyledHeaderTitle, StyledHeaderTitleLink } from './style';
...
const CommPkgTab = ({ commPkg, searchValue, highlightOn }: CommPkgTabProperties): JSX.Element => {
  const navigateToCommPkg = (): void => {
    ...
    url += '/CommPkg?commPkgNo=' + encodeURIComponent(commPkg.commPkg.commPkgNo ?? '') + '&project=' + encodeURIComponent(commPkg.project?.toLocaleUpperCase() ?? '');
      ...
      <StyledCard100>
        <Card.Header>
          <StyledHeaderTitleLink className="link-container">
            ...
            <StyledTypographyLink variant="body_short" onClick={(): void => navigateToCommPkg()}>
              {highlightSearchValue(commPkg.commPkg?.commPkgNo ?? '')}
              <LinkIndicator>
