import { Container, LinkIndicator, StyledCard100, StyledCard50, StyledHeaderTitle, StyledHeaderTitleLink, StyledTypographyLink } from './style';
...
const TagTab = ({ tag: tag, searchValue, highlightOn }: TagTabProperties): JSX.Element => {
  ...
  const highlightSearchValue = (text: string): JSX.Element => {
    ...
    return <Highlighter searchWords={searchValue.split(' ')} autoEscape={true} textToHighlight={text} />;
  };
      ...
      <StyledCard100>
        <Card.Header>
          <StyledHeaderTitleLink className="link-container">
            ...
            <StyledTypographyLink variant="body_short" onClick={(): void => navigateToTag()}>
        ...
        <Card.Header>
          <StyledHeaderTitle>
            ...
            <Typography variant="body_short">
              {highlightSearchValue(tag.tag?.disciplineCode ?? '')}, {highlightSearchValue(tag.tag?.disciplineDescription ?? '')}
            </Typography>
          </StyledHeaderTitle>
        </Card.Header>
      </StyledCard100>
        ...
        <Card.Header>
          {tag.tag?.commPkgNo ? (
            <StyledHeaderTitleLink className="link-container">
              ...
              <StyledTypographyLink variant="body_short" onClick={(): void => (tag.tag?.commPkgNo ? navigateToCommPkg() : undefined)}>
        ...
        <Card.Header>
          {tag.tag?.mcPkgNo ? (
            <StyledHeaderTitleLink className="link-container">
              ...
              <StyledTypographyLink variant="body_short" onClick={(): void => (tag.tag?.mcPkgNo ? navigateToMCPkg() : undefined)}>
