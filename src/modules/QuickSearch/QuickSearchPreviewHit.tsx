...
import { QuickSearchResultItem, SearchResultItemPart, SearchResultType, TypeIndicator } from './style';
...
const QuickSearchPreviewHit = (props: QuickSearchPreviewHitProps): JSX.Element => {
    ...
    const getNumber = (): JSX.Element => {
        const pkgNo = props.item.commPkg ? props.item.commPkg.commPkgNo ?? ''
            : props.item.mcPkg ? props.item.mcPkg.mcPkgNo ?? ''
                : props.item.tag ? props.item.tag.tagNo ?? ''
                    : props.item.punchItem ? props.item.punchItem.punchItemNo ?? ''
                        : '';
    ...
    const getDescription = (): JSX.Element => {
        return (
            highlightSearchValue(
                props.item.commPkg ? props.item.commPkg.description ?? ''
                    : props.item.mcPkg ? props.item.mcPkg.description ?? ''
                        : props.item.tag ? props.item.tag.description ?? ''
                            : props.item.punchItem ? props.item.punchItem.description ?? ''
                                : '')
        );
    };
    ...
    const navigateToItem = (item: ContentDocument): void => {
        ...
        if (item.commPkg) {
            url += "/CommPkg?commPkgNo=" + encodeURIComponent(item.commPkg.commPkgNo ?? '') + "&project=" + getItemProject(item);
        }

        if (item.mcPkg) {
            url += "/MCPkg?mcPkgNo=" + encodeURIComponent(item.mcPkg.mcPkgNo ?? '') + "&project=" + getItemProject(item);
        }

        if (item.tag) {
            url += "/Tag?tagNo=" + encodeURIComponent(item.tag.tagNo ?? '') + "&project=" + getItemProject(item);
        }
    ...
    return (
        <QuickSearchResultItem onClick={(): void => navigateToItem(props.item)}>
            ...
            <SearchResultItemPart group="navigation" variant="menu_title" lines={3}>
                {getDescription()}
            </SearchResultItemPart>
