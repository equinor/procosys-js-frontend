  ...
  async doPreviewSearch(searchString: string, plantId: string, setRequestCanceller?: RequestCanceler): Promise<SearchResult> {
    ...
    try {
      ...
      result.data.items.map((item: ContentDocument) => {
        item.type = item.commPkg ? ResultTypeEnum.COMM_PKG : item.mcPkg ? ResultTypeEnum.MC_PKG : item.tag ? ResultTypeEnum.TAG : item.punchItem ? ResultTypeEnum.PUNCH_ITEM : ResultTypeEnum.OTHER;
      });
      return result.data;
    } catch (error) {
