...
const QuickSearch = (): JSX.Element => {
  ...
  useEffect(() => {
    ...
    if (values && values.query) {
      ...
      if (allPlants === 'true') {
        ...
        setSearchValue(decodeURIComponent(searchVal));
        setSearchInputValue(decodeURIComponent(searchVal));
        return;
      }
          ...
          apiClient
            ...
            .finally(() => {
              ...
              if (values.plant && values.plant.length > 0) {
                if (typeof values.plant === 'string') filteredPlants.push(values.plant as string);
                else {
              ...
              if (values.type && values.type.length > 0) {
                if (typeof values.type === 'string') filteredTypes.push(values.type as string);
                else {
  ...
  const highlightSearchValue = (text: string): JSX.Element => {
    ...
    return <Highlighter searchWords={searchFor} autoEscape={true} textToHighlight={text} />;
  };
  ...
  const getLink = (row: TableOptions<ContentDocument>): JSX.Element => {
    ...
    return (
      <DescriptionCell
        className={currentItem && currentItem.key === doc.key ? 'selected' : ''}
        onClick={(): void => {
          handleItemClick(doc);
        }}
      >
  ...
  const getNumber = (row: TableOptions<ContentDocument>): JSX.Element => {
    const doc = row.value as ContentDocument;
    const pkgNo = doc.commPkg ? (doc.commPkg.commPkgNo ?? '') : doc.mcPkg ? (doc.mcPkg.mcPkgNo ?? '') : doc.tag ? (doc.tag.tagNo ?? '') : doc.punchItem ? (doc.punchItem.punchItemNo ?? '') : '';
  ...
  const getDescription = (row: TableOptions<ContentDocument>): JSX.Element => {
    ...
    return (
      ...
      >
        ...
        >
          {highlightSearchValue(doc.commPkg ? (doc.commPkg.description ?? '') : doc.mcPkg ? (doc.mcPkg.description ?? '') : doc.tag ? (doc.tag.description ?? '') : doc.punchItem ? (doc.punchItem.description ?? '') : '')}
        </ResultCell>
      </DescriptionCell>
    );
  };
  ...
  const columns = [
    ...
    },
    {
      ...
      sortType: (a: UseTableRowProps<ContentDocument>, b: UseTableRowProps<ContentDocument>): 0 | -1 | 1 => {
        const firstValue =
          a.original.type === 'C' ? (a.original.commPkg?.description ?? '') : a.original.type === 'MC' ? (a.original.mcPkg?.description ?? '') : a.original.type === 'T' ? (a.original.tag?.description ?? '') : '';
        const secondValue =
          b.original.type === 'C' ? (b.original.commPkg?.description ?? '') : b.original.type === 'MC' ? (b.original.mcPkg?.description ?? '') : b.original.type === 'T' ? (b.original.tag?.description ?? '') : '';
        if (firstValue > secondValue) return 1;
        else if (firstValue < secondValue) return -1;
        else return 0;
      },
    ...
    <Container>
      <SearchContainer withSidePanel={showFilter && !currentItem}>
        ...
        <TopDiv ref={topDivRef}>
          <QSHeaderDiv>
            ...
            <FlexDiv>
              ...
              <FiltersButton onClick={(): void => toggleShowFilter()} variant={searchAllPlants || (selectedPlants && selectedPlants.length) || (selectedTypes && selectedTypes.length > 0) ? 'contained' : 'ghost'}>
            ...
            <SelectedFilters>
              {selectedPlants &&
                selectedPlants.map((plant) => {
                  return (
                    <FilterChip variant="active" onDelete={(): void => handlePlantRemove(plant)} key={plant}>
                      {plant}
                    </FilterChip>
                  );
                })}
              ...
              {selectedTypes &&
                selectedTypes.map((type) => {
                  return (
                    <FilterChip variant="active" onDelete={(): void => handleTypeRemove(type)} key={type}>
        ...
        <ResultsContainer
          ...
          style={{
            height: 'calc(100% - ' + ((topDivHeight === 0 ? 160 : topDivHeight) + 48) + 'px)',
          }}
        >
          {searching ? (
        ...
        {displayFlyout && currentItem && (
          <StyledSideSheet onClose={(): void => setCurrentItem(undefined)} open={displayFlyout} title={getFlyoutTitle((currentItem as ContentDocument).type ?? '')} variant="large">
